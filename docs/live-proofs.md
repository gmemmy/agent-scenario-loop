...
If the ready-log artifact shows the dev client still loading or bundling JavaScript from Metro, ASL classifies the failure as `dev_client_reload_limbo` with next action `restart_metro_and_reload_dev_client`. Treat that as runtime-environment evidence: restart Metro from the consuming app worktree, reopen the dev-client URL, and rerun before using the scenario for app truth or performance claims.
