...
ASL_RESOURCE_LEASE_DIR=.asl-local/resource-leases
ASL_RESOURCE_LEASE_TTL_MS=120000
ASL_RESOURCE_LEASE_HEARTBEAT_MS=30000
```
...
The command writes a separate simctl capture folder under the selected output root, immediately publishes `raw/ios-simctl-capture-started.json`, seeds the app-owned profile session into native AsyncStorage before launch, then collects stored app profile events after the capture window. Command scenarios seed the scenario command queue through the same storage contract before launch. Command envelopes preserve `id`, `commandId`, `sequence`, `queueId`, command pacing `waitMs`, setup prerequisites in `dependsOnMilestones`, setup readiness milestones that may be queue-less in `unscopedMilestones`, and, for normalized execution-plan commands followed by a milestone wait, `waitForMilestone` plus `waitTimeoutMs`. Deep-link command transport uses the same envelope in query parameters except for storage-only setup prerequisites. Default xcrun/simctl subprocesses are bounded by `--command-timeout-ms` so simulator access failures can finalize health artifacts instead of leaving only planner output. When `raw/ios-profile-events.log` exists, the iOS profile runner ingests that stored truth-event log; otherwise it falls back to `raw/ios-simctl-log.txt`.

If the selected runner also receives `--provider <manifest>`, live adb/simctl capture now schedules provider `startWindow` before the active capture loop, `stopWindow` immediately after it, `afterCapture` only after raw run evidence is staged, and `finalize` for cleanup. For iOS storage-backed command sessions with native-performance live-window providers, ASL first seeds only the app session, waits for same-run profile-session readiness on the final foreground app process, runs provider `startWindow`, and then writes `raw/ios-profile-session-command-release.json` as it releases the command queue into the provider-owned active window. When the configured app profile-session scheme is available, deferred iOS commands are released through app-visible profile-session command links instead of a late external AsyncStorage mutation, while the release artifact preserves the exact expected command envelopes for completion matching. Provider commands may narrow their platform applicability with `platforms`; inapplicable commands and their output demand are omitted rather than failed. Rehydrated `--adb-artifacts`, `--simctl-artifacts`, and fixture/event-log runs stay post-capture-only and fail closed for live-window phases instead of pretending the provider trace overlapped the measured interaction window. Control-only provider phases may declare `outputs: []`; any declared output must be freshly written during that command or ASL rejects it as stale evidence. Evidence-provider manifests at schemaVersion `1.1.0` may also declare `exclusiveResources` for live-window provider ownership. V1 supports only provider identity claims and explicit TCP-port claims, acquires them only after runner-owned target leasing and read-only preflight, then before the first mutable provider phase, and records the path-scrubbed lease journal at `raw/provider-resource-leases.json`. Contention, untrusted acquisition, heartbeat ownership loss, or untrusted release blocks mutable provider work and fails the run while preserving valid partial diagnostics; host-doctor exclusive-process checks stay advisory and do not satisfy provider ownership by themselves. The generated Android native provider resets its measurement window in `startWindow`, writes raw target/gfxinfo/framestats/meminfo outputs in `stopWindow`, and normalizes those exact preserved bytes in `afterCapture` without recollection.
...
When an iOS app exits during the capture window, inspect `raw/ios-app-lifecycle-log.txt` first. ASL checks every host DiagnosticReports candidate whose modification time falls between capture start and inspection, with a five-second clock-skew allowance at each boundary. Bundle identity comes only from a non-empty string `bundleID` in the leading JSON metadata object; matching text in the report body is not identity evidence, and missing or unusable identity makes the search incomplete. Each opened report must retain the same device, inode, size, modification time, and change time before and after its bounded read. The search records `matched`, `not_found`, or `search_incomplete` plus candidate, checked, changed, error, and preview counts in `raw/ios-host-diagnostic-report-search.txt`. Only a stable, complete matching report within the 4 MiB attachment bound is published as `raw/ios-host-diagnostic-report-<bundle>.ips`. A larger stable matching report remains valid crash-association evidence, but its metadata records `attachmentComplete: false`, captured and original byte sizes, and no raw attachment path. Read/stat failures, malformed headers, changed reports, or a truncated non-match make the search incomplete and fail health rather than asserting absence.
...
For a direct Agent Device run outside `asl-live-ios` or `asl-live-android`, keep the same proof graph instead of treating terminal exit zero as product evidence. Run `asl-agent-device --check` with one explicit platform, device identifier, scenario, exact session, `--lease-evidence`, and matching `--lease-run-id`; run the scenario capture with the same target/session/lease/scenario identity; then pass both run directories and a closed sidecar inventory to `asl-adapter-live-proof --request <adapter-live-proof-request.json>`. A lease-enabled capture repeats the exact target/session/scenario availability gate before it creates output or sends mutable commands. The bridge requires the selected target and session to appear in Agent Device capability/session evidence, requires the lease journal to remain active for the exact platform, target, owner, and lease run, and confirms that preflight covered every capture command. The request declares each expected recording, screenshot, UI tree, action transcript, log, metric, health, verdict, or summary as required or optional and as present or not available. ASL hashes present files from the capture root. Missing or invalid required evidence fails the platform proof, while unavailable optional evidence stays visible. A passed bridge is mechanism proof with the adapter verdict still `not_evaluated`; app-owned state is required for product acceptance.

The resulting platform proof can be combined with the other required platform through `asl-live-proof`, then supplied to `asl-ci-evidence-pack assemble` through its existing request contract. When a bounded shareable directory is needed, call `asl-evidence-package --request <evidence-package-request.json>` with an explicit allowlist of already-redacted proof, pack, media, and diagnostic files. The materializer does not traverse the source root, never copies undeclared files, and rejects sensitive path names, private-key markers, symlinks, and output reuse. Inspect `evidence-package.json` and `SHA256SUMS`; do not treat their presence as a product, publication, or release verdict.
...
The comparison gate is intentionally strict. If either run failed scenario health, or if the scenario ids do not match, the comparison is `inconclusive`. Numeric budget checks are compared only after that health gate passes. `comparison.json` includes `comparisonBasis` with the baseline/current run ids and run directories, giving agents artifact-local provenance instead of forcing them to infer it from folder names. It also includes `measurementPolicy`, which records the baseline selection mode, poisoning protections, valid sample counts, timing tolerance, and confidence level used for the comparison. Each run contributes at most one valid sample when it contains finite comparable budget evidence. Distinct budget metrics do not count as repeated samples, so ordinary comparison confidence remains single-run unless a future explicit comparison contract supplies repeated-sample truth. SchemaVersion `1.1.0` introduced the optional `nativePerformance` section; the current `1.2.0` writer preserves it and adds scenario-contract compatibility. When trusted baseline/current native evidence also proves the same-condition contract, that section records per-metric deltas, direction, percent change, and separate budget status. If the contract cannot be proven, the section stays `not-comparable` with explanations instead of upgrading diagnostics into a claim.

The latest-trusted command excludes the exact current run directory from baseline selection. Baseline trust requires passed health and passed verdict. It also requires artifact schemas the current reader understands: missing legacy schema versions remain legacy-compatible, but malformed schema versions or future major versions are indexed as `artifact_schema_incompatible` and cannot seed baselines. For attempt-aware artifacts, baseline trust also requires a clean first passed attempt, no retry lineage, no failed or partial cleanup, and no valid partial-artifact diagnostic fragments. Current runs must pass scenario health before the command will compare timing or budget evidence. If the current manifest declares `comparisonLane`, baseline selection is scoped to trusted prior runs with the same lane; if the current manifest has no lane, selection stays within unlabeled trusted prior runs. Profile manifests also include `scenarioHash`, a stable fingerprint of the normalized scenario contract. Exact hash matches remain preferred. A current scenario may explicitly accept exact prior hashes through `acceptedBaselineScenarioHashes`; that declaration is directional, same-scenario only, and non-transitive. A hash-aware current run rejects missing, mismatched, or undeclared baseline hashes. Legacy runs without a current hash retain legacy selection behavior without being labeled exact. This keeps proof modes such as plain live proof and live proof plus agent-device sidecar from comparing against each other, while allowing a deliberate compatible scenario evolution without silently poisoning before/after verdicts. Direct, latest-trusted, and native-performance comparison use the same compatibility resolver. Latest-trusted artifacts set `comparisonBasis.strategy` to `latest_trusted_prior`, record the selected scenario-contract relationship and selection counts for inspected, trusted, trusted-prior, lane-comparable, and scenario-contract-comparable candidates, and mirror the active lane, scenario hash, compatibility relationship, and cohort hash inside `measurementPolicy.baselineSelection.poisoningProtection` when those filters are active.
