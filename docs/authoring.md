...
Use `kind: "nativePerformance"` for platform-native render, frame, memory, or trace summaries. Android examples include Perfetto, trace-processor summaries, `gfxinfo`/framestats, `meminfo`, and logcat-derived render signals. iOS examples include Instruments, `xctrace`, MetricKit, OS signposts, or simulator-derived native performance summaries. JSON native-performance outputs are schema-validated and should preserve provider identity, tool metadata, target binding, lifecycle, completeness, comparability, and at least one content surface such as frames, memory, metrics, traces, diagnostic source inventory, or attachments. Keep raw traces attached, keep provenance explicit, and mark the evidence diagnostic-only until the scenario has a stable cohort and comparison policy for native metrics.

When a native-performance provider combines multiple source files or tools, add `diagnosticSources` entries for each source. Record `captured`, `partial`, `failed`, `timed-out`, `unsupported`, `missing`, or `redacted` per source instead of collapsing the entire native evidence lane into a single success or failure. This is especially important for video-heavy or continuously animating screens where one provider surface can time out while frame, trace, or memory evidence remains useful for diagnosis.
