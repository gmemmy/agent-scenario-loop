...
Use `kind: "nativePerformance"` for platform-native render, frame, memory, or trace summaries. Android examples include Perfetto, trace-processor summaries, `gfxinfo`/framestats, `meminfo`, and logcat-derived render signals. iOS examples include Instruments, MetricKit, or simulator-derived native performance summaries. Keep raw traces attached, keep provenance explicit, and mark the evidence diagnostic-only until the scenario has a stable cohort and comparison policy for native metrics.
