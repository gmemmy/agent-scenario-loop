...
Diagnostic inventory entries also carry `availability`. Use that field to route the next action: `captured` means the surface is present and can satisfy the declared diagnostic requirement, `captured-diagnostic-only` means evidence survived but cannot satisfy a required claim because the run/provider is unhealthy or the native-performance envelope is not complete, comparable, comparison-sufficient, and target-verified, `provider-blocked` points at provider command output, `unsupported` points at provider or runner capability selection, `environment-blocked` points at host/runtime setup, and the missing/not-requested values distinguish scenario intent from absent optional evidence.
