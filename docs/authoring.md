...
The generated starter provider offers opt-in native capture lanes. Android uses `ASL_NATIVE_PERFORMANCE_ANDROID_CAPTURE=1` and requires an explicit package id plus device serial for bounded no-shell `adb` diagnostics. iOS Simulator uses `ASL_NATIVE_PERFORMANCE_IOS_CAPTURE=1` and requires an explicit bundle id plus simulator UDID for bounded no-shell `xcrun simctl` target checks and `xcrun xctrace record/export --toc` capture. The generated scaffold now preserves lifecycle entrypoints, target-binding state, immutable command records, and the optional `{nativePerformanceRequestPath}` plus `{nativePerformanceRequestSha256}` inputs so a project-local provider can move real native capture into `startWindow` and `stopWindow`, validate the runner-owned identity/window contract up front, and keep normalization in `afterCapture`; its built-in adb and xctrace sample lanes still perform their actual capture work in `afterCapture`, so they remain diagnostic-only even on live adb/simctl runs. Both lanes preserve command argv/stdout/stderr/exit-timeout evidence and keep failures or partials visible; iOS additionally preserves run-relative TOC, target-binding proof, and trace-bundle inventory files because `.trace` captures are directory bundles. These lanes can become complete enough for diagnosis when every requested step succeeds, but they remain diagnostic-only and must not claim comparison readiness from trace-window metadata or post-window normalization alone.
