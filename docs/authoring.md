...
| `templates/scripts/asl-capture-native-performance-provider.mjs` | Starter native-performance provider with scaffold/input ingestion and opt-in Android adb diagnostics |
...
The generated starter provider offers an opt-in Android `adb` path through `ASL_NATIVE_PERFORMANCE_ANDROID_CAPTURE=1`. It requires an explicit package id and device serial, bounds each no-shell command, attempts all configured sources after individual failures, preserves raw command evidence, and verifies target binding from durable serial/package observations. Its output remains partial and diagnostic-only because after-capture gfxinfo, framestats, and meminfo summaries do not establish a bounded provider-owned measurement window. The generated iOS provider remains scaffold/input ingestion; use the Android path as one-platform diagnostic evidence, not as complete mobile coverage.
