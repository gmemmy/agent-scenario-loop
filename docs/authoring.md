...
Provider scripts that use the native-performance helper APIs can pass explicit `claimSufficiency`, `comparability`, `targetBinding`, and `completenessStatus` overrides instead of hand-building JSON envelopes. Use those overrides to keep partial native evidence useful while making the missing or ambiguous surface visible. The helper still validates the final artifact against the schema, so comparison-ready claims must satisfy the same complete/comparable/verified requirements as hand-authored evidence.
