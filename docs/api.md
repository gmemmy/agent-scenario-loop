...
`materializeEvidencePackage(request)` is the package-root API for building an allowlist-first, owner-private local evidence directory. `verifyEvidencePackage(packageDir)` reopens a completed directory and verifies its manifest, checksums, contained files, classified JSON references, and the schema `1.1.0` completion marker when applicable; verification continues to work after relocation. The request and output contracts are shipped as `schemas/evidence-package-request.schema.json` and `schemas/evidence-package.schema.json`, registered as `SCHEMAS.evidencePackageRequest` and `SCHEMAS.evidencePackage`. Version `1.0.0` retains its request shape, remains marker-free, and performs opaque byte copy and byte verification. It does not parse JSON, classify paths, or require valid JSON based on filename. Schema `1.1.0` performs typed JSON-path classification plus marker-sealed completeness. It requires `jsonPointers`, which may be empty when parsed packaged JSON contains no absolute host path. `artifact-reference` values name another requested source entry and are rewritten to its packaged `files/...` path, while `host-local-provenance` values remain unchanged and are recorded as non-dereferenceable. Only string values in parsed schema `1.1.0` packaged JSON are inspected for absolute host paths. Non-JSON files and prose inside them are byte inventory, not interpreted path contracts; that proof boundary does not permit secrets. Other path-like strings, including shell expansion syntax and file URLs, are not classified as absolute filesystem paths by this contract. Duplicate semantic JSON-pointer records fail closed. The materializer stable-reads only declared regular files, requires portable output-relative destinations under `files/`, rejects path traversal, portable path collisions, symlinks, missing or empty files, invalid declared schema `1.1.0` JSON, selected sensitive environment/key paths, and selected private-key markers, and never replaces an existing output directory. It verifies private staged bytes, atomically reserves a previously absent output directory, and moves the staged payload into that reservation. Files, checksums, and `evidence-package.json` are written first. Schema `1.1.0` requires `completionMarkerPath` `evidence-package.complete`; after those files are in final output it fsyncs every sealed artifact file, `SHA256SUMS`, `evidence-package.json`, and the directories needed to persist the files tree, then exclusively creates that marker mode `0600`, fsyncs the marker, and fsyncs the output directory. Marker bytes are lowercase SHA-256 of the `evidence-package.json` bytes plus a trailing newline. For schema `1.1.0`, `SHA256SUMS` inventories the marker, the manifest, and the packaged entries. That marker is the last durable completion seal: a `1.1.0` directory without it is incomplete even if the manifest, checksums, and payload exist. Directory presence or manifest status alone is insufficient. Only `verifyEvidencePackage` successfully checking the exact marker, manifest, checksums, and closed inventory proves a schema `1.1.0` directory complete. Verification requires the exact marker bytes and checksum for schema `1.1.0` only; a missing or drifted marker fails closed. Failed publication removes package-owned incomplete output and newly created empty parent directories. `EvidencePackageError.rejections` is the structured local failure inventory. Package completion and verification prove portable byte inventory and declared reference roles only; they are not general secret scanning or product, runtime, publication, deployment, or release acceptance.
...
TypeScript consumers can import `CiEvidencePack` types and the named builder, reader, derivation, verifier, assembler, and parse helpers from the package root. The assembler public API is `assembleCiEvidencePack`, `verifyCiEvidencePackLiveProofSet`, `CiEvidencePackAssemblyOptions`, `VerifiedCiEvidencePackLiveProofPointer`, and `VerifiedCiEvidencePackLiveProofSet`. `buildCiEvidencePack` and `readCiEvidencePack` validate the pack schema and cross-record semantics. `parseCiEvidencePackBytes(bytes)` admits exact pack bytes and throws `CiEvidencePackError` for fatal UTF-8, JSON, schema, or derived-semantic failures. `verifyCiEvidencePackLiveProofSet` and `assembleCiEvidencePack` read and hash-verify the live-proof-set file; child proof and summary pointers must resolve to distinct regular files inside the real artifact root. The assembler verifies pointer identity and containment but does not read or semantically interpret child bytes. Stable contained-file I/O stays internal to the assembler module and is not a package-root export. Emitted packs are not rewritten; retries remain distinct retained attempts. That immutability is artifact lifecycle, not a JavaScript deep-freeze. Mechanism status, the derived platform evidence claim, product verdict, comparison, publication, release, and deployment remain distinct. Current writer schema is `1.1.0` and requires `platformScope` (`single-platform` or `cross-platform`) plus `platformClaim`; it never emits `twoPlatformClaim`. `single-platform` requires exactly one required platform (`android` or `ios`). `cross-platform` requires exactly `android` and `ios`. Single-platform iOS evidence proves only iOS and is not mobile or cross-platform complete. Undeclared platforms are out of scope, not missing, unsupported, or `not_evaluable`. A missing declared platform fails closed; the writer does not fabricate another platform. `not_applicable` remains authored pre-runtime scenario applicability vocabulary only. Legacy `1.0.0` packs remain readable as cross-platform and retain `twoPlatformClaim`; readers do not rewrite them. `twoPlatformClaim` is legacy 1.0 reader vocabulary only. The schema is shipped at `agent-scenario-loop/schemas/ci-evidence-pack.schema.json` and registered as `SCHEMAS.ciEvidencePack`.
