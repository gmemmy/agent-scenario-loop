...
`materializeEvidencePackage(request)` is the package-root API for building an allowlist-first, owner-private local evidence directory. `verifyEvidencePackage(packageDir)` reopens a completed directory and verifies its manifest, checksums, contained files, and classified JSON references after relocation. The request and output contracts are shipped as `schemas/evidence-package-request.schema.json` and `schemas/evidence-package.schema.json`, registered as `SCHEMAS.evidencePackageRequest` and `SCHEMAS.evidencePackage`. Version `1.0.0` retains its request shape and byte-copies JSON that has no absolute host paths. JSON containing an absolute POSIX, Windows-drive, or UNC path must use version `1.1.0` and explicit `jsonPointers`: `artifact-reference` values name another requested source entry and are rewritten to its packaged `files/...` path, while `host-local-provenance` values remain unchanged and are recorded as non-dereferenceable. Other path-like strings, including shell expansion syntax and file URLs, are not classified as absolute filesystem paths by this contract. The materializer stable-reads only declared regular files, requires portable output-relative destinations under `files/`, rejects path traversal, portable path collisions, symlinks, missing or empty files, invalid declared JSON, selected sensitive environment/key paths, and selected private-key markers, and never replaces an existing output directory. It verifies private staged bytes, atomically reserves a previously absent output directory, moves the staged payload into that reservation, and installs the hash-bearing `evidence-package.json` completion manifest last alongside `SHA256SUMS`. Failed publication removes package-owned incomplete output and newly created empty parent directories. `EvidencePackageError.rejections` is the structured local failure inventory. Package completion and verification prove portable byte inventory and declared reference roles only; they are not general secret scanning or product, runtime, publication, deployment, or release acceptance.
