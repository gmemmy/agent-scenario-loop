...
- `evidence-package.json`: allowlist-first inventory for an owner-private evidence directory with narrow path and private-key screens
...
Direct Agent Device captures can enter the same aggregate contract through `adapter-live-proof-request.schema.json`. The bridge request must bind one exact target-aware availability run to an exact session and trusted active lease, one same-scenario capture run, and a closed declaration of expected sidecars. A capture supplied with lease evidence requires one exact target selector and an explicit session, and it repeats target-bound availability before creating output or sending mutable work. The bridge verifies canonical health, verdict, summary, target-binding, session, lease-run, scenario, flow, and sidecar file identities before writing. It also requires the preflight command inventory to cover every command declared by the capture metadata; a generic legacy capture identity or unscoped adapter-health check is not admissible. Standalone Agent Device checks retain the broad legacy command inventory, and standalone captures may retain their legacy unleased mode, but those paths are not admissible to this bridge. A lease rejected before probing is recorded as `not-probed`, not as fabricated capability or help evidence. Each sidecar is retained as `present`, `missing`, `invalid`, `not_available`, or `rejected`; a required status other than `present` fails the platform live proof. Optional unavailable evidence remains visible without failing an otherwise healthy mechanism proof. A passed direct-adapter live proof means the declared runner evidence is identity-bound and complete. It does not convert the linked adapter verdict from `not_evaluated`, establish app-owned selected state, or prove depicted product behavior.

`evidence-package.json` inventories only files explicitly named by an `evidence-package-request.schema.json` request. Source and destination paths must be safe run-relative POSIX paths, and destinations must stay under `files/`. Request and artifact `schemaVersion` `1.0.0` preserve their original shape, remain marker-free, and perform opaque byte copy and byte verification: they do not parse JSON, classify paths, or require valid JSON based on filename. Schema `1.1.0` is typed JSON-path classification plus marker-sealed completeness. It requires `completionMarkerPath` `evidence-package.complete` and `jsonPointers`, which may be empty when parsed packaged JSON contains no absolute host path; every declared JSON pointer has one closed role. An `artifact-reference` names a second requested source entry; the materializer confirms the original absolute value identifies that entry and rewrites the packaged value to the entry's `files/...` path. `host-local-provenance` preserves an absolute execution path as explicitly non-dereferenceable metadata. Only string values in parsed schema `1.1.0` packaged JSON are inspected for absolute host paths; every absolute POSIX, Windows-drive, or UNC path string in those values must be classified. Non-JSON files and prose inside them are byte inventory, not interpreted path contracts; that proof boundary does not permit secrets. Shell expansion syntax, drive-relative strings, and file URLs are outside this filesystem-path vocabulary. Duplicate semantic JSON-pointer records fail closed.

The materializer stable-reads each regular non-symlink source file, rejects duplicates and portable destination collisions, missing or empty files, traversal, invalid or missing pointers, non-string pointer targets, missing referenced entries, selected sensitive environment/key paths, and selected private-key markers, then writes and verifies owner-private staged bytes. Rewritten JSON byte sizes and hashes describe the packaged bytes, while source artifacts remain unchanged. Publication atomically reserves a previously absent output directory and moves the staged payload into that reservation. Files, checksums, and `evidence-package.json` land first. Schema `1.1.0` then fsyncs every sealed artifact file, `SHA256SUMS`, `evidence-package.json`, and the directories needed to persist the files tree before it exclusively creates `evidence-package.complete` mode `0600`, fsyncs the marker, and fsyncs the output directory. Marker bytes are lowercase SHA-256 of the `evidence-package.json` bytes plus a trailing newline. For schema `1.1.0`, `SHA256SUMS` inventories the marker, the manifest, and the packaged entries. That marker is the last durable completion seal: a `1.1.0` directory without it is incomplete even if the manifest, checksums, and payload exist. Version `1.0.0` remains marker-free; a `1.0.0` directory without both `evidence-package.json` and `SHA256SUMS` is not a complete package. Any rejection yields structured failure and no canonical package directory. Existing output is never replaced, and failed publication removes package-owned incomplete output plus newly created empty parent directories. Unlisted files are never traversed or copied.

`verifyEvidencePackage(packageDir)` validates the public manifest, exact checksum inventory, closed package file inventory, every declared file's size and digest, every classified pointer, and every referenced packaged entry from the package's current location. Marker verification applies to schema `1.1.0` only and requires the exact `evidence-package.complete` bytes and checksum; a missing or drifted marker fails closed. Directory presence or manifest status alone is insufficient: only `verifyEvidencePackage` successfully checking the exact marker, manifest, checksums, and closed inventory proves a schema `1.1.0` directory complete. Verification therefore continues to work after the producer root is deleted and the package is relocated. Host-local provenance is never opened as a package path. Completion and verification prove portable byte inventory, containment, checksums, and declared reference roles only; they are not general secret scanning and do not prove product, runtime, comparison, publication, release, or deployment acceptance.

Platform-set proof commands write `live-proof-set.json` and `agent-summary.md` under the caller-provided proof-set output directory. The proof-set artifact records required platforms, present platforms, missing platforms, each linked `live-proof.json`, failed proof reasons, regression-gate reasons, and a next action. When required platform proofs are missing, the proof set may include `missingPlatformNextActions` with owner/code counts per missing platform so readers can separate coverage collection from failed proof, warning, and skipped sidecar follow-up. When linked proofs fail or trigger an active regression gate, the proof set may include `proofFailureNextActions` with owner/code counts from those proof next-action hints, so readers can separate failed proof or regression follow-up ownership from skipped sidecar blockers. If failed linked proofs carry diagnostic sufficiency pairs through skipped sidecar declarations, the proof set may include `proofFailureDiagnosticSufficiency` with failed proof count plus captured and blocking per-kind/status counts so provider-blocked and diagnostic-only evidence on failed proofs is visible separately from optional warning and skipped-gate summaries. If failed linked proofs carry requested diagnostic inventory through skipped sidecar declarations, the proof set may include `proofFailureRequestedDiagnostics` with failed proof count plus requested diagnostic counts by kind, status, and requiredness so requested-but-missing diagnostics are visible separately from optional unrequested surfaces. If failed linked proofs carry native-performance claim context through skipped sidecar declarations, the proof set may include `proofFailureNativePerformance` with failed proof count plus claim sufficiency, completeness, comparability, target-binding, target-binding detail, and diagnostic source counts so native-performance claim blockers on failed proofs are visible separately from skipped-gate summaries. If failed linked proofs carry preserved provider-evidence next actions through skipped sidecar declarations, the proof set may include `proofFailureProviderEvidenceNextActions` with failed proof count plus owner/code counts so provider-tooling follow-up is visible without opening each failed profile folder. If failed linked proofs carry profile-session readiness context through skipped sidecar declarations, the proof set may also include `proofFailureReadiness` with failed proof count, skipped proof count, failure class, command total, dev-client deep-link, foreground probe, seed, pending phase, expected-evidence, and derived readiness-detail counts so no-command, storage seed, deep-link, and foreground-target command-channel blockers are visible without opening every platform proof first. If failed linked proofs carry readiness next actions through skipped sidecar declarations, the proof set may include `proofFailureReadinessNextActions` with failed proof count plus owner/code counts for runtime-environment follow-up. When linked proofs include optional interaction warnings with next-action hints, the proof set may include `interactionWarningNextActions` with owner/code counts for those warning checks, so partial sidecar evidence remains visible without upgrading warnings into failed proof gates. When linked proofs include skipped sidecar declarations, the proof set may include `skippedInteractionProofCount` and flattened `skippedInteractionProofs` entries with platform, linked proof, reason, next action, and any profile-gate diagnostics or readiness context already carried by the child proof. It may also include `skippedInteractionProofNextActions` with owner/code counts from those skipped proof next-action hints, so readers can see whether blockers concentrate in runtime environment, app truth, provider tooling, runner, scenario contract, or optimization follow-up. When those skipped proofs include diagnostic sufficiency pairs, the proof set may also include `profileGateDiagnosticSufficiency` with captured and blocking per-kind/status counts so readers can see provider partial-evidence shape across platforms. When skipped proof gates include native-performance claim context, the proof set may include `profileGateNativePerformance` with claim sufficiency, completeness, comparability, target-binding, target-binding detail, and diagnostic source counts for those skipped gates. When skipped proof gates include preserved provider-evidence next actions, the proof set may include `profileGateProviderEvidenceNextActions` with owner/code counts for those skipped gates. When skipped proof gates include profile-session readiness context, the proof set may include `profileGateReadiness` with failure class, command total, dev-client deep-link, foreground probe, seed, pending phase, expected-evidence, and derived readiness-detail counts for those skipped gates. When skipped proof gates include readiness next actions, the proof set may include `profileGateReadinessNextActions` with owner/code counts for those skipped gates. When linked proofs include `profileNativePerformance`, the proof set may also include a combined `profileNativePerformance` rollup with profile/evidence counts, diagnostic source status counts, completeness status counts, claim sufficiency counts, comparability counts, target-binding counts, and optional target-binding detail counts across platforms. This gives agents one stable index over linked per-platform live proofs after those proofs have written their own aggregate evidence; the proof-set rollups remain indexes over linked profile evidence, not new product-performance claims. `live-proof-set` stays schema `1.0.0` and already records required, present, and missing platforms. It does not carry `platformScope`.

Current `ci-evidence-pack.json` (`schemaVersion` 1.1.0) is a runner-neutral wrapper over exactly one live-proof-set reference. It does not replace the proof set. Current artifacts require `platformScope` of `single-platform` or `cross-platform` and a derived `platformClaim`. `single-platform` requires exactly one required platform, `android` or `ios`. `cross-platform` requires exactly `android` and `ios`. Current writers never emit `twoPlatformClaim`. Single-platform iOS evidence proves only iOS and must not be called mobile-complete or cross-platform complete. An undeclared platform is out of scope, not missing, unsupported, or `not_evaluable`. A missing declared platform fails closed; the pack never fabricates another platform. Legacy `1.0.0` artifacts remain readable as cross-platform and retain `twoPlatformClaim`; readers do not rewrite them. `twoPlatformClaim` is legacy 1.0 reader vocabulary only. The PR1 builder and reader validate the pack schema and cross-record semantics. The runner-independent assembler reads and hash-verifies the live-proof-set file, then checks SHA-256, byte size, schema, platform inventory, and selected-attempt run identity before building the pack. Child proof and summary pointers must resolve to distinct regular files inside the real artifact root. The assembler verifies pointer identity and containment but does not read or semantically interpret child bytes. Existing live-proof-set child pointers may be absolute host paths; the assembler accepts those pointers only when they are contained by the declared artifact root and does not copy them into the pack. Stable live-proof-set reading is the hash-bound file read; this slice does not claim broader TOCTOU guarantees over later child-path races. `liveProofSet` is one hash-bearing reference; the assembler does not interpret product verdicts. Emitted packs are not rewritten; retries remain distinct retained attempts. That immutability is artifact lifecycle, not a JavaScript deep-freeze. The pack preserves exact source, platform, attempt, and evidence identities, plus run-relative hash-bound evidence. Failed and rejected attempts remain visible; a selected retry does not erase them. Missing required declared platform, stale or mismatched source, and invalid or incomplete evidence fail closed. Selected unsupported authority is `not_evaluable`, never retroactive `not_applicable`. `not_applicable` remains authored pre-runtime scenario applicability vocabulary only. Artifact presence proves only that artifact's obligation; product verdict remains the linked verdict truth. Mechanism status, the derived platform evidence claim, product verdict, comparison, publication, runtime, release, and deployment stay separate. The pack itself does not carry publication, release, or deployment fields.

Current `ci-evidence-publication-receipt.json` (`schemaVersion` 1.1.0) is a separate public contract that binds one immutable CI evidence pack by exact SHA-256 digest and exact byte size. Legacy receipts remain `schemaVersion` 1.0.0. Receipt `schemaVersion` and the nested pack `schemaVersion` must match: a 1.1.0 receipt binds a 1.1.0 pack, and a 1.0.0 receipt binds a 1.0.0 pack. Do not mix current and legacy versions on one receipt. Current packs keep `schemaVersion` 1.1.0 and current fields (`platformScope`, `platformClaim`); legacy packs keep `schemaVersion` 1.0.0 and legacy fields (`twoPlatformClaim`). `live-proof-set` stays schema `1.0.0`. Assembly, publication, publication-gate evaluation, product verdict, comparison, release, deployment, and runtime acceptance remain distinct proof tiers. It records publication mechanics only: destination identity, attempt identity, and `publicationStatus` of `published`, `partial`, `failed`, or `not_published`. It does not rewrite pack mechanism status, the platform evidence claim, product verdict, comparison, release, or deployment. Publication success proves only that the exact pack bytes were published as recorded; it is not product success, comparison success, release acceptance, deployment, or runtime acceptance.

`readCiEvidencePublicationReceipt(receiptPath, exactPackBytes)` is the exact-byte authority: it parses the supplied pack bytes after local semantic admission. `assertCiEvidencePublicationReceiptForExactPackBytes(receipt, parsedPack, exactPackBytes)` then binds an already-admitted parsed pack to those same bytes so unknown evidence ids, non-present published evidence, copied identity, sha256, and byteSize fail closed. `assertCiEvidencePublicationReceiptForPack` validates semantic and copied binding only; it does not hash pack bytes. Do not treat a ForPack-only check as byte-identity proof. Receipt publication cannot rewrite mechanism, platform claim, product, comparison, completeness, assembly, release, deployment, or runtime truth.

The public renderer signature is `renderCiEvidencePublicationSummary(pack, receipt, exactPackBytes)`. Generic render behavior remains public-link strict and does not expose restricted URLs. The evaluator signature is `evaluateCiEvidencePublicationSummary(pack, receipt, exactPackBytes, options?)`. Default and explicit `options.audience='public'` require safe HTTPS public visibility. `options.audience='authenticated_reviewer'` permits safe HTTPS public or restricted outcomes so reviewer-link obligations can pass without treating restricted destinations as public. Receipt admission, exact-byte binding, non-present evidence rejection, and `publicationStatus` remain unchanged. Neither audience mode proves mechanism, platform evidence claim, product, runtime, comparison, release, or deployment acceptance. This is audience policy only, not GitHub upload or API behavior. Invalid binding or unsafe published destinations throw `CiEvidencePublicationReceiptError`. Publication success does not prove deployment.

Failed, rejected, private, not-available, and invalid publication outcomes remain visible retained records. A later successful receipt does not erase them. Destinations are generic safe HTTPS URLs; this contract does not encode GitHub-specific upload, workflow, or PR reporting behavior. The deterministic Markdown reviewer summary requires the same exact pack bytes as the receipt and must surface distinct publication and platform evidence claim status fields without claiming product success. Summary and report wording is platform evidence claim and must state exact platform scope. Upload, PR reporting, workflow orchestration, release acceptance, and deployment remain later separate proof layers.

The public `asl-ci-evidence-pack` CLI is a runner-independent assemble/summarize/github-report phase over those same contracts. Assembly, publication transport, publication-gate evaluation, product verdict, comparison, release, deployment, and runtime acceptance remain distinct proof layers. Restricted authenticated-reviewer HTTPS links are reviewer-facing evidence, not public publication. `assemble --request <request.json>` accepts the exact closed request envelope `{ artifactRoot, outDir, input }` where `input` is the exact closed `CiEvidencePackBuildInput` shape; extra top-level or `input` keys are usage failures. The assembler consumes existing artifacts: it does not run a device, upload anything, call GitHub, publish a release, deploy, or claim product, runtime, comparison, release, or deployment acceptance. The closed assemble request has no runner-identity field; runner/provider identity remains in the hash-bound live-proof-set and linked proof/provider artifacts. The CI pack does not synthesize or reinterpret that identity. Assemble exit 0 requires current source, a passed live proof set, complete evidence, succeeded assembly/mechanism, and a passed derived platform evidence claim for the declared `platformScope`. Assemble stdout fields are `phase`, `artifact`, `publicationAttempted` (`false`), `gateStatus`, `mechanismStatus`, `platformScope`, `requiredPlatforms`, and `platformClaimStatus`. It must not emit `twoPlatformClaimStatus`. Comparison status and product verdict remain separate planes and do not gate pack assembly. If assembly returns a schema-valid nonpassing pack, the CLI writes it and then exits 1. Verification, assembler, or I/O exceptions exit 1 without claiming or writing a canonical pack. Exit 2 for usage or request-shape failure and must not create an output pack. Failed and retried attempts remain visible retained records; missing declared evidence or platform fails closed and stays distinct from an undeclared out-of-scope platform. Stale, mismatched, invalid, or incomplete evidence also fails closed.

`summarize --pack <ci-evidence-pack.json> --receipt <ci-evidence-publication-receipt.json> [--out <summary.md>]` reads pack bytes once, binds the receipt and deterministic Markdown summary to those exact bytes, and writes the summary atomically. `--out` resolving to the pack or receipt is usage exit 2 before any reads or writes and cannot clobber either input. Exit 0 only when publication-summary evaluation is passing. A schema-valid nonpassing summary is still written and then exits 1. Verification, renderer, or I/O exceptions exit 1 without claiming a passing summary. Summarize does not rewrite pack mechanism status, the derived platform evidence claim, selected product verdicts, comparison, release, runtime acceptance, or deployment.

`github-report --pack <ci-evidence-pack.json> --input <github-publication-input.json> --out <new-directory>` is a local-only GitHub reviewer-report materialization over one immutable pack and restricted authenticated-reviewer publication facts. Pack bytes are read once and bound to the generated `ci-evidence-publication-receipt.json` and `ci-evidence-github-report.md`. `--out` is an exclusively reserved new directory: existing files, directories, symlinks, dangling symlinks, pack/input aliases, and races are never replaced. Occupied or alias `--out` is usage exit 2 and preserves existing bytes. An incomplete marker (`.ci-evidence-github-report.incomplete`) is written first; canonical receipt and Markdown names are committed only after both files are complete, then the marker is removed. Exit 0 only for a passing publication gate. A valid failed publication or source fixture still writes both canonical files for inspection and exits 1 with stdout `phase`, `artifactDirectory`, `receiptArtifact`, `reportArtifact`, and `gateStatus` `failed`. github-report stdout does not emit `artifact`, `productVerdictStatus`, or `comparisonStatus`; product verdict and comparison remain separate non-gates in the pack/report contract. Malformed, unsafe, or I/O failure exits 1, or usage 2 as defined; interrupted or failed writes may retain a marker-bearing visibly incomplete directory and do not promise that no directory remains. The command does not upload, call the GitHub API, create checks, publish a release, deploy, run a device, or claim product, runtime, comparison, release, or deployment acceptance. Artifact presence and publication-gate success prove only that those obligations were recorded; they do not prove depicted product behavior. Restricted authenticated-reviewer links must not be described as public visibility.
...
Legacy `schemaVersion` 1.0.0 publication receipts remain readable with their historical `twoPlatformClaim` summary and published next-action wording. Current `schemaVersion` 1.1.0 receipts use `platform evidence claim` and `platformScope` summary wording.
