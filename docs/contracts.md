...
When a provider command fails after writing some declared outputs, ASL keeps the captured diagnostics in the manifest and emits `partial_provider_evidence_preserved` as a warning health check. Those outputs are diagnostic-only until missing required outputs are fixed and health passes.
