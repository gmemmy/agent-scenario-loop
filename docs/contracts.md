...
- `steps[].driverAction`: optional concrete driver operation required by a step, such as `tap`, `longPress`, `swipe`, `scroll`, `drag`, `pinch`, `rotate`, `customGesture`, `typeText`, `pressKey`, `pressButton`, `runSequence`, `assertVisible`, `inspectTree`, `screenshot`, `record`, `readLogs`, or `collectPerfSignals`
...
Runner capabilities describe ownership, such as launch, session control, command execution, log capture, artifact writing, or profiler support. Driver actions describe the concrete operations an adapter can perform inside a run. UI contexts describe which surface the runner or provider can own: `app`, `systemDialog`, `notificationShade`, `externalBrowser`, `webView`, `nativePreview`, `shareSheet`, `picker`, or `otherApp`. UI and capture driver actions default to `app` when a step omits `uiContext`; a scenario must opt into system or external contexts explicitly. A runner may be able to own a scenario lifecycle without supporting every driver action or UI context; the planner fails when a required step declares a `driverAction` or `uiContext` that the selected runner or an active provider does not declare.

The driver-action vocabulary is intentionally wider than the built-in dispatch port. Tools such as Argent expose custom gestures, pinch, rotate, hardware buttons, keyboard input, and fixed run sequences, while adb/simctl own smaller subsets. ASL lets runner and provider manifests declare those richer actions without forcing every built-in adapter to implement them. Execution support is still proven by the selected runner or provider; unsupported actions must fail compatibility or return explicit unsupported evidence.
...
Long-press is a portable input action. By default it is app UI like other UI driver actions, and it can also be paired with explicit UI contexts for system-owned surfaces, context menus, selection affordances, drag handles, or native previews. Native preview behavior is one such capability contract, not an assumed tap fallback: use `driverAction: "longPress"` for the input gesture and `uiContext: "nativePreview"` for platform-owned preview or preview-menu assertions outside normal app UI. If no active runner or provider declares the required action and context, planner health should fail or warn according to the step's requiredness instead of treating tap, generic context-menu, or policy-readiness evidence as native preview proof.
...
When a provider command fails after writing some declared outputs, ASL keeps the captured diagnostics in the manifest and emits `partial_provider_evidence_preserved` as a warning health check. Agent summaries also index those preserved diagnostic paths separately from failed checks so agents can use them for diagnosis without treating the run as a product claim. Those outputs are diagnostic-only until missing required outputs are fixed and health passes.
