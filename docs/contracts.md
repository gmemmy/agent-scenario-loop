...
`evidence-package.json` inventories only files explicitly named by an `evidence-package-request.schema.json` request. Source and destination paths must be safe run-relative POSIX paths, and destinations must stay under `files/`. Request and artifact `schemaVersion` `1.0.0` preserve their original shape and byte-copy JSON without absolute host paths. JSON containing an absolute path must use version `1.1.0`, which requires every declared JSON pointer to have one closed role. An `artifact-reference` names a second requested source entry; the materializer confirms the original absolute value identifies that entry and rewrites the packaged value to the entry's `files/...` path. `host-local-provenance` preserves an absolute execution path as explicitly non-dereferenceable metadata. Every absolute POSIX, Windows-drive, or UNC path string in packaged JSON must be classified; shell expansion syntax, drive-relative strings, and file URLs are outside this filesystem-path vocabulary.

The materializer stable-reads each regular non-symlink source file, rejects duplicates and portable destination collisions, missing or empty files, traversal, invalid or missing pointers, non-string pointer targets, missing referenced entries, selected sensitive environment/key paths, and selected private-key markers, then writes and verifies owner-private staged bytes. Rewritten JSON byte sizes and hashes describe the packaged bytes, while source artifacts remain unchanged. Publication atomically reserves a previously absent output directory, moves the staged payload into that reservation, and installs `evidence-package.json` last alongside `SHA256SUMS`; a directory without both seals is not a complete package. Any rejection yields structured failure and no canonical package directory. Existing output is never replaced, and failed publication removes package-owned incomplete output plus newly created empty parent directories. Unlisted files are never traversed or copied.

`verifyEvidencePackage(packageDir)` validates the public manifest, exact checksum inventory, closed package file inventory, every declared file's size and digest, every classified pointer, and every referenced packaged entry from the package's current location. Verification therefore continues to work after the producer root is deleted and the package is relocated. Host-local provenance is never opened as a package path. Completion and verification prove portable byte inventory, containment, checksums, and declared reference roles only; they are not general secret scanning and do not prove product, runtime, comparison, publication, release, or deployment acceptance.
