...
`evidence-package.json` (`schemaVersion` 1.0.0) inventories only files explicitly named by an `evidence-package-request.schema.json` request. Source and destination pointers must be safe run-relative POSIX paths, and destination pointers must stay under `files/`. The materializer stable-reads each regular non-symlink source file, rejects duplicates and portable destination collisions, missing or empty files, traversal, selected sensitive environment/key paths, and selected private-key markers, then writes and verifies owner-private staged bytes. Publication atomically reserves a previously absent output directory, moves the staged payload into that reservation, and installs `evidence-package.json` last alongside `SHA256SUMS`; a directory without that manifest is not a complete package. Any rejected requested entry yields a structured rejection and no canonical package directory. Existing output is never replaced, and failed publication removes package-owned incomplete output plus newly created empty parent directories. Unlisted files are never traversed or copied, including files outside the artifact inventory. Package completion proves byte inventory, containment, and the declared narrow sensitivity checks only; the materializer is not a general secret scanner, does not semantically redact arbitrary contents, and does not prove product, runtime, comparison, publication, release, or deployment acceptance.
