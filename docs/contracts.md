...
`evidence-package.json` inventories only files explicitly named by an `evidence-package-request.schema.json` request. Source and destination paths must be safe run-relative POSIX paths, and destinations must stay under `files/`. Request and artifact `schemaVersion` `1.0.0` preserve their original shape, remain marker-free, and byte-copy JSON without absolute host paths. JSON containing an absolute path must use version `1.1.0`, which requires `completionMarkerPath` `evidence-package.complete` and every declared JSON pointer to have one closed role. An `artifact-reference` names a second requested source entry; the materializer confirms the original absolute value identifies that entry and rewrites the packaged value to the entry's `files/...` path. `host-local-provenance` preserves an absolute execution path as explicitly non-dereferenceable metadata. Only string values in packaged JSON are inspected for absolute host paths; every absolute POSIX, Windows-drive, or UNC path string in those values must be classified. Non-JSON files and prose inside them are byte inventory, not interpreted path contracts; that proof boundary does not permit secrets. Shell expansion syntax, drive-relative strings, and file URLs are outside this filesystem-path vocabulary. Duplicate semantic JSON-pointer records fail closed.
