...
Native preview behavior is a capability contract, not an assumed tap fallback. Use `driverAction: "longPress"` for the input gesture and `uiContext: "nativePreview"` for platform-owned preview or preview-menu surfaces that are outside normal app UI. If no active runner or provider declares `longPress` and `nativePreview`, planner health should fail or warn according to the step's requiredness instead of treating tap, context-menu, or policy-readiness evidence as native preview proof.
