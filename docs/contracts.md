...
- [runner/validate-project.ts](../runner/validate-project.ts): initialized project validator for app helper presence, package-script snippets, app `package.json` script merge and direct-bin drift, config presence, scenario manifests, runner manifests, and planner compatibility
...
- `project-validation.json`: project-level validation result for initialized app scaffolds, including helper readiness, package-script snippet readiness, app `package.json` script merge and direct-bin drift readiness, non-failing setup warnings, and structured next actions
