...
`ci-evidence-pack.json` (`schemaVersion` 1.0.0) is an immutable runner-neutral wrapper over exactly one validated `live-proof-set.json`. It does not replace the proof set. The pack preserves exact source, platform, attempt, and evidence identities, plus run-relative hash-bound evidence. Failed and rejected attempts remain visible; a selected retry does not erase them. Missing required platform, stale or mismatched source, and invalid or incomplete evidence fail closed. Selected unsupported authority is `not_evaluable`, never retroactive `not_applicable`. Artifact presence proves only that artifact's obligation; product verdict remains the linked verdict truth. Mechanism status, the derived Android-plus-iOS claim, and product truth stay separate. Upload, PR reporting, release acceptance, deployment, and publication receipts are later separate proof layers and are not fields on this pack.
