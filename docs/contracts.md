...
Native performance evidence uses the `nativePerformance` kind. This is separate from `profiler`: use it for platform-native frame, render, memory, and trace summaries such as Android Perfetto, trace-processor output, `gfxinfo`/framestats, `meminfo`, logcat-derived render signals, iOS Instruments, `xctrace`, MetricKit, OS signposts, or simulator-derived native summaries. A provider can mark `nativePerformance` output as required, and the profile manifest will preserve it as a required diagnostic when captured. JSON native-performance outputs are validated against ASL's native-performance evidence schema, so they must include provider, platform, run, scenario, and at least one content surface such as summary, metrics, frames, memory, events, traces, diagnostic source inventory, or attachments. Raw traces should remain attached evidence while structured summaries carry the claim-ready facts, provenance, and comparability status.

Use `diagnosticSources` inside native-performance JSON when a provider has multiple platform-native inputs with different outcomes. Each entry names a `sourceKind` such as `perfetto`, `trace-processor`, `gfxinfo`, `framestats`, `meminfo`, `logcat-render`, `xctrace`, `instruments`, `metrickit`, `simctl-log`, or `os-signpost`, plus a status such as `captured`, `partial`, `failed`, `timed-out`, `unsupported`, `missing`, or `redacted`. This lets agents preserve useful native or profiler-adjacent evidence when another surface failed, without pretending the missing source exists or that diagnostic-only evidence is budget-comparable.
