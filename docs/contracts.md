...
When a provider command fails after writing some declared outputs, ASL keeps the captured diagnostics in the manifest and emits `partial_provider_evidence_preserved` as a warning health check. Its metadata includes `capturedKinds`, `capturedPaths`, `blockingRequiredKinds`, and `blockingRequiredStatuses` when available, so agents can separate useful diagnostic evidence from required surfaces that still block completeness. Agent summaries also index those preserved diagnostic paths separately from failed checks so agents can use them for diagnosis without treating the run as a product claim. Those outputs are diagnostic-only until missing required outputs are fixed and health passes.
