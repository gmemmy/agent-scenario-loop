...
`ci-evidence-pack.json` (`schemaVersion` 1.0.0) is a runner-neutral wrapper over exactly one live-proof-set reference. It does not replace the proof set. The PR1 builder and reader validate the pack schema and cross-record semantics. `liveProofSet` is one hash-bearing reference to a caller-validated live-proof-set; PR1 does not read or verify linked live-proof-set bytes. Linked byte, hash, and inventory verification belong to a later assembler. Emitted packs are not rewritten; retries remain distinct retained attempts. That immutability is artifact lifecycle, not a JavaScript deep-freeze. The pack preserves exact source, platform, attempt, and evidence identities, plus run-relative hash-bound evidence. Failed and rejected attempts remain visible; a selected retry does not erase them. Missing required platform, stale or mismatched source, and invalid or incomplete evidence fail closed. Selected unsupported authority is `not_evaluable`, never retroactive `not_applicable`. Artifact presence proves only that artifact's obligation; product verdict remains the linked verdict truth. Mechanism status, the derived Android-plus-iOS evidence claim, product verdict, comparison, publication, release, and deployment stay separate. Upload, PR reporting, release acceptance, deployment, and publication receipts are later separate proof layers and are not fields on this pack.
