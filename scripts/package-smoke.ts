...
function main(): void {
  ...
  try {
    ...
    const resolveSmokeScript = [
      ...
      "  assert.doesNotMatch(iosOnlyGitHubMarkdown, /Android selected product verdict/);",
      ...
      "  const readLegacyPack = asl.readCiEvidencePackBytes ? asl.readCiEvidencePackBytes(Buffer.from(JSON.stringify(legacyPack))) : asl.parseCiEvidencePackBytes(Buffer.from(JSON.stringify(legacyPack)));",
      "  assert.equal(readLegacyPack.schemaVersion, '1.0.0');",
      "  assert.equal(readLegacyPack.twoPlatformClaim.status, 'passed');",
      "  assert.equal('platformScope' in readLegacyPack, false);",
      "  assert.equal('platformClaim' in readLegacyPack, false);",
      "} finally {",
