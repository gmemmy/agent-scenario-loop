...
function main(): void {
  ...
  try {
    ...
    const resolveSmokeScript = [
      ...
      "assert.deepEqual(asl.DRIVER_PORT, ['tap', 'longPress', 'scroll', 'assertVisible', 'inspectTree', 'screenshot', 'record', 'readLogs', 'collectPerfSignals']);",
      ...
      "assert.deepEqual(driverValidation.missingMethods, ['longPress', 'scroll', 'assertVisible', 'inspectTree', 'record', 'readLogs', 'collectPerfSignals']);",
      "assert.equal(driverValidation.message, 'driver is missing required method(s): longPress, scroll, assertVisible, inspectTree, record, readLogs, collectPerfSignals');",
