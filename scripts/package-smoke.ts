...
function main(): void {
  ...
  try {
    ...
    const resolveSmokeScript = [
      ...
      "assert.deepEqual(asl.DRIVER_PORT, ['tap', 'longPress', 'typeText', 'scroll', 'swipe', 'pinch', 'assertVisible', 'inspectTree', 'screenshot', 'record', 'readLogs', 'collectPerfSignals']);",
      ...
      "assert.deepEqual(driverValidation.missingMethods, ['longPress', 'typeText', 'scroll', 'swipe', 'pinch', 'assertVisible', 'inspectTree', 'record', 'readLogs', 'collectPerfSignals']);",
      "assert.equal(driverValidation.message, 'driver is missing required method(s): longPress, typeText, scroll, swipe, pinch, assertVisible, inspectTree, record, readLogs, collectPerfSignals');",
