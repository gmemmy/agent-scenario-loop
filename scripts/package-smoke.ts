...
const PACKED_FILE_ALLOWLIST = [
  ...
  /^app\/profile-session-helper\.json$/u,
...
function main(): void {
  ...
  try {
    ...
    const resolveSmokeScript = [
      ...
      "assert.equal(asl.CI_EVIDENCE_PACK_SCHEMA_VERSION, '1.1.0');",
      ...
      "assert.equal(asl.CI_EVIDENCE_PUBLICATION_RECEIPT_SCHEMA_VERSION, '1.1.0');",
      ...
      "  assert.equal(readCiPack.schemaVersion, '1.1.0');",
      "  assert.equal(readCiPack.platformScope, 'cross-platform');",
      "  assert.equal(readCiPack.platformClaim.status, 'passed');",
      "  assert.deepEqual(readCiPack.platformClaim.reasons, []);",
      "  assert.equal('twoPlatformClaim' in readCiPack, false);",
      ...
      "  assert.match(publicationSummary, /\\| platform evidence claim \\| passed \\|/);",
      ...
      "  assert.equal(assembleStdout.platformScope, 'cross-platform');",
      "  assert.deepEqual(assembleStdout.requiredPlatforms, ['android', 'ios']);",
      "  assert.equal(assembleStdout.platformClaimStatus, 'passed');",
      "  assert.equal('twoPlatformClaimStatus' in assembleStdout, false);",
      ...
      "  assert.equal(cliPack.schemaVersion, '1.1.0');",
      "  assert.equal(cliPack.platformScope, 'cross-platform');",
      "  assert.equal(cliPack.platformClaim.status, 'passed');",
      "  assert.equal('twoPlatformClaim' in cliPack, false);",
      ...
      "  assert.equal(parsedCliPack.platformClaim.status, 'passed');",
      "  assert.equal(parsedCliPack.platformScope, 'cross-platform');",
      "  assert.equal('twoPlatformClaim' in parsedCliPack, false);",
      ...
      "  assert.throws(() => asl.parseCiEvidencePackBytes(Buffer.from(JSON.stringify({ schemaVersion: '1.1.0' }))), asl.CiEvidencePackError);",
      ...
      "  assert.match(cliSummary, /\\| platform evidence claim \\| passed \\|/);",
      "  assert.match(cliSummary, /cross-platform/);",
      "  assert.doesNotMatch(cliSummary, /two-platform evidence claim/);",
      ...
      "  const missingDeclaredDir = path.join(ciPackArtifactRoot, 'missing-declared-platform');",
      "  fs.mkdirSync(missingDeclaredDir, { recursive: true });",
      "  const missingDeclaredRequestPath = path.join(ciPackArtifactRoot, 'missing-declared-platform-request.json');",
      "  const missingDeclaredPackPath = path.join(missingDeclaredDir, 'ci-evidence-pack.json');",
      "  const missingDeclaredInput = JSON.parse(JSON.stringify(ciPackInput));",
      "  missingDeclaredInput.packId = 'package-smoke-ci-pack-missing-declared';",
      "  missingDeclaredInput.platforms = [missingDeclaredInput.platforms[0]];",
      "  missingDeclaredInput.attempts = missingDeclaredInput.attempts.filter((attempt) => attempt.platform === 'android');",
      "  missingDeclaredInput.evidence = missingDeclaredInput.evidence.filter((item) => item.platform === 'android');",
      "  missingDeclaredInput.verdicts = missingDeclaredInput.verdicts.filter((verdict) => verdict.platform === 'android');",
      "  fs.writeFileSync(missingDeclaredRequestPath, JSON.stringify({ artifactRoot: ciPackArtifactRoot, outDir: missingDeclaredDir, input: missingDeclaredInput }));",
      "  const missingDeclaredResult = runCiPackCli(['assemble', '--request', missingDeclaredRequestPath]);",
      "  assert.equal(missingDeclaredResult.status, 1, missingDeclaredResult.stderr || missingDeclaredResult.stdout);",
      "  assert.equal(fs.existsSync(missingDeclaredPackPath), true);",
      "  const missingDeclaredStdout = JSON.parse(missingDeclaredResult.stdout);",
      "  assert.equal(missingDeclaredStdout.gateStatus, 'failed');",
      "  assert.equal(missingDeclaredStdout.platformClaimStatus, 'failed');",
      "  assert.equal('twoPlatformClaimStatus' in missingDeclaredStdout, false);",
      "  const missingDeclaredPack = asl.readCiEvidencePack(missingDeclaredPackPath);",
      "  assert.equal(missingDeclaredPack.schemaVersion, '1.1.0');",
      "  assert.equal(missingDeclaredPack.platformClaim.status, 'failed');",
      "  assert.equal(missingDeclaredPack.requiredPlatforms.includes('ios'), true);",
      "  assert.equal(missingDeclaredPack.platformClaim.reasons.some((reason) => /ios/.test(reason)), true);",
      "  assert.equal('twoPlatformClaim' in missingDeclaredPack, false);",
      "  const iosOnlyRoot = path.join(ciPackArtifactRoot, 'ios-only');",
      "  fs.mkdirSync(iosOnlyRoot, { recursive: true });",
      "  const iosOnlyLiveProofSetPath = path.join(iosOnlyRoot, 'live-proof-set.json');",
      "  const iosOnlyProofPointer = ciPackProofPointer('ios', 'ios-only-pass');",
      "  iosOnlyProofPointer.runId = 'run-ios-only';",
      "  iosOnlyProofPointer.filePath = path.join(iosOnlyRoot, 'ios-only-pass', 'live-proof.json');",
      "  iosOnlyProofPointer.summaryPath = path.join(iosOnlyRoot, 'ios-only-pass', 'summary.json');",
      "  const iosOnlyLiveProofSet = {",
      "    schemaVersion: '1.0.0',",
      "    runId: 'run-ios-only',",
      "    status: 'passed',",
      "    proofCount: 1,",
      ...
      "    missingPlatforms: [],",
      "    proofs: [iosOnlyProofPointer],",
      "    failureReasons: [],",
      "    nextAction: { code: 'none', summary: 'No action required.' },",
      "    summary: 'iOS live proof assembled.'",
      "  };",
      "  fs.mkdirSync(path.dirname(iosOnlyProofPointer.filePath), { recursive: true });",
      "  fs.writeFileSync(iosOnlyProofPointer.filePath, JSON.stringify({ platform: 'ios', runId: 'run-ios-only' }));",
      "  fs.writeFileSync(iosOnlyProofPointer.summaryPath, JSON.stringify({ platform: 'ios', runId: 'run-ios-only', kind: 'summary' }));",
      "  fs.writeFileSync(iosOnlyLiveProofSetPath, JSON.stringify(iosOnlyLiveProofSet));",
      "  const iosOnlyLiveProofSetBytes = fs.readFileSync(iosOnlyLiveProofSetPath);",
      "  const iosOnlyLiveProofSetSha256 = crypto.createHash('sha256').update(iosOnlyLiveProofSetBytes).digest('hex');",
      "  const iosOnlyEvidence = ciPackAttemptEvidence('ios', 'ios-only-pass');",
      "  const iosOnlyInput = {",
      "    schemaVersion: '1.1.0',",
      "    packId: 'package-smoke-ci-pack-ios-only',",
      "    createdAt: '2026-08-22T00:07:00.000Z',",
      "    source: ciPackSource,",
      "    liveProofSet: { relativePath: 'live-proof-set.json', sha256: iosOnlyLiveProofSetSha256, byteSize: iosOnlyLiveProofSetBytes.length, runId: 'run-ios-only', status: 'passed' },",
      "    platformScope: 'single-platform',",
      "    requiredPlatforms: ['ios'],",
      "    requiredEvidenceKinds: ['recording', 'verdict'],",
      "    platforms: [{ platform: 'ios', authorityStatus: 'supported', evaluationStatus: 'passed', selectedAttemptId: 'ios-only-pass' }],",
      "    attempts: [{ attemptId: 'ios-only-pass', platform: 'ios', scenarioId: 'scenario-a', runId: 'run-ios-only', status: 'passed', attemptNumber: 1, maxAttempts: 1, startedAt: '2026-08-22T00:07:00.000Z', evidenceIds: iosOnlyEvidence.evidenceIds }],",
      "    evidence: iosOnlyEvidence.evidence,",
      "    verdicts: [{ scenarioId: 'scenario-a', runId: 'run-ios-only', platform: 'ios', status: 'passed', evidenceId: 'ios-only-pass-verdict' }],",
      "    comparisonStatus: 'not_available',",
      "    completeness: { status: 'complete', reasons: [] },",
      "    assembly: { status: 'succeeded', reasons: [] },",
      "    summary: 'ios evidence assembled',",
      "    nextAction: 'publish receipt in a later slice'",
      "  };",
      "  asl.verifyCiEvidencePackLiveProofSet(iosOnlyInput, { artifactRoot: iosOnlyRoot });",
      "  const iosOnlyCliOutDir = path.join(iosOnlyRoot, 'cli-out');",
      "  fs.mkdirSync(iosOnlyCliOutDir, { recursive: true });",
      "  const iosOnlyRequestPath = path.join(iosOnlyRoot, 'assemble-request.json');",
      "  fs.writeFileSync(iosOnlyRequestPath, JSON.stringify({ artifactRoot: iosOnlyRoot, outDir: iosOnlyCliOutDir, input: iosOnlyInput }));",
      "  const iosOnlyPackPath = path.join(iosOnlyCliOutDir, 'ci-evidence-pack.json');",
      "  const iosOnlyAssemble = runCiPackCli(['assemble', '--request', iosOnlyRequestPath]);",
      "  assert.equal(iosOnlyAssemble.status, 0, iosOnlyAssemble.stderr || iosOnlyAssemble.stdout);",
      "  const iosOnlyAssembleStdout = JSON.parse(iosOnlyAssemble.stdout);",
      "  assert.equal(iosOnlyAssembleStdout.gateStatus, 'passed');",
      "  assert.equal(iosOnlyAssembleStdout.mechanismStatus, 'succeeded');",
      "  assert.equal(iosOnlyAssembleStdout.platformScope, 'single-platform');",
      "  assert.deepEqual(iosOnlyAssembleStdout.requiredPlatforms, ['ios']);",
      "  assert.equal(iosOnlyAssembleStdout.platformClaimStatus, 'passed');",
      "  assert.equal('twoPlatformClaimStatus' in iosOnlyAssembleStdout, false);",
      "  const iosOnlyPack = JSON.parse(fs.readFileSync(iosOnlyPackPath, 'utf8'));",
      "  assert.equal(iosOnlyPack.schemaVersion, '1.1.0');",
      "  assert.equal(iosOnlyPack.platformScope, 'single-platform');",
      "  assert.deepEqual(iosOnlyPack.requiredPlatforms, ['ios']);",
      "  assert.equal(iosOnlyPack.platformClaim.status, 'passed');",
      "  assert.equal('twoPlatformClaim' in iosOnlyPack, false);",
      "  assert.deepEqual(iosOnlyPack.platforms.map((row) => row.platform), ['ios']);",
      "  assert.equal(iosOnlyPack.attempts.every((attempt) => attempt.platform === 'ios'), true);",
      "  assert.equal(iosOnlyPack.evidence.every((item) => item.platform === 'ios'), true);",
      "  assert.equal(iosOnlyPack.verdicts.every((verdict) => verdict.platform === 'ios'), true);",
      "  const iosOnlyPackBytes = fs.readFileSync(iosOnlyPackPath);",
      "  const iosOnlyPackSha256 = crypto.createHash('sha256').update(iosOnlyPackBytes).digest('hex');",
      "  const iosOnlyReceipt = asl.buildCiEvidencePublicationReceipt({",
      "    packBytes: iosOnlyPackBytes,",
      "    facts: {",
      "      receiptId: 'package-smoke-ci-publication-receipt-ios-only',",
      "      createdAt: '2026-08-22T00:08:00.000Z',",
      "      packRelativePath: 'ci-evidence-pack.json',",
      "      publisher: { providerId: 'package-smoke', providerKind: 'local', runId: 'package-smoke-run-ios', attemptNumber: 1 },",
      "      requestedItems: [",
      "        { requestId: 'pack-artifact-ci-evidence-pack', targetKind: 'pack_artifact', packArtifact: 'ci_evidence_pack' },",
      "        { requestId: 'pack-artifact-live-proof-set', targetKind: 'pack_artifact', packArtifact: 'live_proof_set' },",
      "        { requestId: 'evidence-ios-only-pass-recording', targetKind: 'evidence', evidenceId: 'ios-only-pass-recording' },",
      "        { requestId: 'evidence-ios-only-pass-verdict', targetKind: 'evidence', evidenceId: 'ios-only-pass-verdict' }",
      "      ],",
      "      outcomes: [",
      "        { requestId: 'pack-artifact-ci-evidence-pack', status: 'published', url: 'https://evidence.example.test/packs/package-smoke-ci-pack-ios-only', visibility: 'public', publishedAt: '2026-08-22T00:08:00.000Z' },",
      "        { requestId: 'pack-artifact-live-proof-set', status: 'published', url: 'https://evidence.example.test/packs/package-smoke-live-proof-set-ios-only', visibility: 'public', publishedAt: '2026-08-22T00:08:00.000Z' },",
      "        { requestId: 'evidence-ios-only-pass-recording', status: 'published', url: 'https://evidence.example.test/evidence/ios-only-pass-recording', visibility: 'public', publishedAt: '2026-08-22T00:08:00.000Z' },",
      "        { requestId: 'evidence-ios-only-pass-verdict', status: 'published', url: 'https://evidence.example.test/evidence/ios-only-pass-verdict', visibility: 'public', publishedAt: '2026-08-22T00:08:00.000Z' }",
      "      ]",
      "    }",
      "  });",
      "  asl.assertCiEvidencePublicationReceiptForExactPackBytes(iosOnlyReceipt, iosOnlyPack, iosOnlyPackBytes);",
      "  assert.equal(iosOnlyReceipt.schemaVersion, '1.1.0');",
      "  assert.equal(iosOnlyReceipt.pack.schemaVersion, '1.1.0');",
      "  assert.equal(iosOnlyReceipt.pack.sha256, iosOnlyPackSha256);",
      "  const iosOnlyReceiptPath = path.join(iosOnlyCliOutDir, 'ci-evidence-publication-receipt.json');",
      "  fs.writeFileSync(iosOnlyReceiptPath, JSON.stringify(iosOnlyReceipt));",
      "  const iosOnlySummaryPath = path.join(iosOnlyCliOutDir, 'publication-summary.md');",
      "  const iosOnlySummarize = runCiPackCli(['summarize', '--pack', iosOnlyPackPath, '--receipt', iosOnlyReceiptPath, '--out', iosOnlySummaryPath]);",
      "  assert.equal(iosOnlySummarize.status, 0, iosOnlySummarize.stderr || iosOnlySummarize.stdout);",
      "  const iosOnlySummary = fs.readFileSync(iosOnlySummaryPath, 'utf8');",
      "  assert.match(iosOnlySummary, /\\| platform evidence claim \\| passed \\|/);",
      "  assert.match(iosOnlySummary, /single-platform/);",
      "  assert.match(iosOnlySummary, /iOS selected product verdict/);",
      "  assert.doesNotMatch(iosOnlySummary, /Android selected product verdict/);",
      "  assert.doesNotMatch(iosOnlySummary, /cross-platform/);",
      "  assert.doesNotMatch(iosOnlySummary, /two-platform evidence claim/);",
      "  assert.doesNotMatch(iosOnlySummary, /mobile complete/i);",
      "  assert.match(iosOnlySummary, /Publication success does not prove runtime acceptance\./);",
      "  const iosOnlyGitHubFacts = {",
      "    schemaVersion: '1.0.0',",
      "    receiptId: 'package-smoke-github-publication-ios-only',",
      ...
      "      providerKind: 'ci_workflow',",
      "      providerId: 'github-actions',",
      "      runId: 'package-smoke-run-ios-only',",
      "      workflowId: 'ci-evidence.yml',",
      "      jobId: 'publish-evidence',",
      "      attemptNumber: 1",
      "    },",
      ...
      "    ],",
      "    outcomes: [",
      "      { requestId: 'pack-artifact-ci-evidence-pack', status: 'published', url: 'https://github.com/example-org/example-repo/actions/artifacts/pack-ios', visibility: 'restricted', publishedAt: '2026-08-22T00:09:00.000Z' },",
      "      { requestId: 'pack-artifact-live-proof-set', status: 'published', url: 'https://github.com/example-org/example-repo/actions/artifacts/live-proof-set-ios', visibility: 'restricted', publishedAt: '2026-08-22T00:09:00.000Z' },",
      "      { requestId: 'evidence-ios-only-pass-recording', status: 'published', url: 'https://github.com/example-org/example-repo/actions/artifacts/ios-only-pass-recording', visibility: 'restricted', publishedAt: '2026-08-22T00:09:00.000Z' },",
      "      { requestId: 'evidence-ios-only-pass-verdict', status: 'published', url: 'https://github.com/example-org/example-repo/actions/artifacts/ios-only-pass-verdict', visibility: 'restricted', publishedAt: '2026-08-22T00:09:00.000Z' }",
      "    ]",
      "  };",
      "  const iosOnlyGitHubInputPath = path.join(iosOnlyRoot, 'github-publication-input.json');",
      "  fs.writeFileSync(iosOnlyGitHubInputPath, JSON.stringify(iosOnlyGitHubFacts));",
      "  const iosOnlyGitHubOutDir = path.join(iosOnlyRoot, 'github-report-out');",
      "  const iosOnlyGitHubResult = runCiPackCli(['github-report', '--pack', iosOnlyPackPath, '--input', iosOnlyGitHubInputPath, '--out', iosOnlyGitHubOutDir]);",
      "  assert.equal(iosOnlyGitHubResult.status, 0, iosOnlyGitHubResult.stderr || iosOnlyGitHubResult.stdout);",
      "  const iosOnlyGitHubStdout = JSON.parse(iosOnlyGitHubResult.stdout);",
      "  assert.equal(iosOnlyGitHubStdout.gateStatus, 'passed');",
      "  const iosOnlyGitHubMarkdown = fs.readFileSync(path.join(iosOnlyGitHubOutDir, 'ci-evidence-github-report.md'), 'utf8');",
      "  assert.match(iosOnlyGitHubMarkdown, /\\| platform evidence claim \\| passed \\|/);",
      "  assert.match(iosOnlyGitHubMarkdown, /single-platform/);",
      "  assert.match(iosOnlyGitHubMarkdown, /\\| ios \\| supported \\| passed \\| ios-only-pass \\| passed \\|/);",
      "  assert.doesNotMatch(iosOnlyGitHubMarkdown, /\\| android \\|/);",
      "  assert.doesNotMatch(iosOnlyGitHubMarkdown, /cross-platform/);",
      "  assert.doesNotMatch(iosOnlyGitHubMarkdown, /two-platform evidence claim/);",
      "  assert.doesNotMatch(iosOnlyGitHubMarkdown, /mobile complete/i);",
      "  const iosOnlyGitHubReceipt = asl.readCiEvidencePublicationReceipt(path.join(iosOnlyGitHubOutDir, 'ci-evidence-publication-receipt.json'), iosOnlyPackBytes);",
      "  asl.assertCiEvidencePublicationReceiptForExactPackBytes(iosOnlyGitHubReceipt, iosOnlyPack, iosOnlyPackBytes);",
      "  assert.equal(iosOnlyGitHubReceipt.schemaVersion, '1.1.0');",
      "  assert.equal(iosOnlyGitHubReceipt.pack.schemaVersion, '1.1.0');",
      "  assert.match(iosOnlyGitHubMarkdown, /do not prove product, runtime, comparison, release, deployment, or merge acceptance\./);",
      "  const legacyPack = JSON.parse(JSON.stringify(readCiPack));",
      "  legacyPack.schemaVersion = '1.0.0';",
      "  delete legacyPack.platformScope;",
      "  delete legacyPack.platformClaim;",
      "  legacyPack.twoPlatformClaim = { status: 'passed', reasons: [] };",
      "  const legacyPackPath = path.join(ciPackArtifactRoot, 'legacy-ci-evidence-pack.json');",
      "  const legacyPackBytes = Buffer.from(JSON.stringify(legacyPack));",
      "  fs.writeFileSync(legacyPackPath, legacyPackBytes);",
      "  const readLegacyPackFromPath = asl.readCiEvidencePack(legacyPackPath);",
      "  const parsedLegacyPack = asl.parseCiEvidencePackBytes(legacyPackBytes);",
      "  assert.deepEqual(readLegacyPackFromPath, parsedLegacyPack);",
      "  assert.equal(readLegacyPackFromPath.schemaVersion, '1.0.0');",
      "  assert.equal(readLegacyPackFromPath.twoPlatformClaim.status, 'passed');",
      "  assert.equal('platformScope' in readLegacyPackFromPath, false);",
      "  assert.equal('platformClaim' in readLegacyPackFromPath, false);",
      "  assert.deepEqual(readLegacyPackFromPath.requiredPlatforms, ['android', 'ios']);",
      "  assert.deepEqual(fs.readFileSync(legacyPackPath), legacyPackBytes);",
      "  assert.throws(() => asl.parseCiEvidencePackBytes(Buffer.from(JSON.stringify({ ...readCiPack, twoPlatformClaim: readCiPack.platformClaim }))), asl.CiEvidencePackError);",
      "  assert.throws(() => asl.parseCiEvidencePackBytes(Buffer.from(JSON.stringify({ ...legacyPack, platformScope: 'cross-platform' }))), asl.CiEvidencePackError);",
      "  assert.throws(() => asl.parseCiEvidencePackBytes(Buffer.from(JSON.stringify({ ...legacyPack, platformClaim: legacyPack.twoPlatformClaim }))), asl.CiEvidencePackError);",
      ...
      "assert.equal(fs.existsSync('node_modules/agent-scenario-loop/app/profile-session-helper.json'), true);",
      "require.resolve('agent-scenario-loop/app/profile-session');",
      "const profileSessionHelperJsonPath = require.resolve('agent-scenario-loop/app/profile-session-helper.json');",
      "const profileSessionHelperIdentity = JSON.parse(fs.readFileSync(profileSessionHelperJsonPath, 'utf8'));",
      "assert.deepEqual(Object.keys(profileSessionHelperIdentity).sort(), ['payloadId', 'payloadSha256', 'version']);",
      "assert.equal(profileSessionHelperIdentity.version, '1.2.0');",
      "assert.equal(profileSessionHelperIdentity.payloadId, 'agent-scenario-loop/profile-session-helper@1.2.0+idempotent-owned-stop');",
      "assert.equal(profileSessionHelperIdentity.payloadSha256, '2ba19944de0d94271a27a99c4188e8567dce0d67b3e0df5ddb5f311108ce8178');",
      "const profileSessionHelperExport = packageJson.exports['./app/profile-session-helper.json'];",
      "assert.equal(profileSessionHelperExport.import, './app/profile-session-helper.json');",
      "assert.equal(profileSessionHelperExport.require, './app/profile-session-helper.json');",
      "assert.equal(profileSessionHelperExport.default, './app/profile-session-helper.json');",
      "const profileSessionRuntime = fs.readFileSync('node_modules/agent-scenario-loop/app/profile-session.ts', 'utf8');",
      "assert.match(profileSessionRuntime, /from ['\"]\\.\\/profile-session-helper\\.json['\"]/u);",
      "assert.equal(/PROFILE_SESSION_HELPER_VERSION\\s*=\\s*['\"]1\\.2\\.0['\"]/u.test(profileSessionRuntime), false);",
      "assert.equal(/PROFILE_SESSION_HELPER_PAYLOAD_ID\\s*=\\s*['\"]agent-scenario-loop\\/profile-session-helper@1\\.2\\.0\\+idempotent-owned-stop['\"]/u.test(profileSessionRuntime), false);",
      "assert.equal(/PROFILE_SESSION_HELPER_PAYLOAD_SHA256\\s*=\\s*['\"]2ba19944de0d94271a27a99c4188e8567dce0d67b3e0df5ddb5f311108ce8178['\"]/u.test(profileSessionRuntime), false);",
      ...
      "assert.match(profileSessionDeclarations, /PROFILE_SESSION_HELPER_VERSION: ['\"]1\\.2\\.0['\"]/u);",
      "assert.match(profileSessionDeclarations, /PROFILE_SESSION_HELPER_PAYLOAD_ID: ['\"]agent-scenario-loop\\/profile-session-helper@1\\.2\\.0\\+idempotent-owned-stop['\"]/u);",
      "assert.match(profileSessionDeclarations, /PROFILE_SESSION_HELPER_PAYLOAD_SHA256: ['\"]2ba19944de0d94271a27a99c4188e8567dce0d67b3e0df5ddb5f311108ce8178['\"]/u);",
