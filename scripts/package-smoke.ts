...
function main(): void {
  ...
  try {
    ...
    const typeSmokeSource = [
      ...
      "const invalidClaimResult: ClaimResult = { claimId: 'claim', claimHash: 'hash', role: 'mandatory', status: 'supported', reasonCode: 'health_gate_failed', assertionResults: [{ assertionId: 'assertion', status: 'not_evaluable', reasonCode: 'health_gate_failed', expected: null, observed: null }], evidenceReferences: [], missingProof: [], nextActionOwner: 'unresolved', nextAction: 'inspect evidence' };",
      "// @ts-expect-error rejected results require authoritative_evidence_rejected.",
      "const invalidRejectedClaimResult: ClaimResult = { claimId: 'claim', claimHash: 'hash', role: 'mandatory', status: 'rejected', reasonCode: 'partial_evidence', assertionResults: [{ assertionId: 'assertion', status: 'not_evaluable', reasonCode: 'partial_evidence', expected: null, observed: null }], evidenceReferences: [], missingProof: [], nextActionOwner: 'unresolved', nextAction: 'inspect evidence' };",
      "// @ts-expect-error not_evaluable results cannot use supported reason vocabulary.",
      "const invalidNotEvaluableClaimResult: ClaimResult = { claimId: 'claim', claimHash: 'hash', role: 'mandatory', status: 'not_evaluable', reasonCode: 'all_assertions_supported', assertionResults: [{ assertionId: 'assertion', status: 'not_evaluable', reasonCode: 'partial_evidence', expected: null, observed: null }], evidenceReferences: [], missingProof: [], nextActionOwner: 'unresolved', nextAction: 'inspect evidence' };",
      "// @ts-expect-error assertion results use the same status and reason correlation.",
      "const invalidAssertionResult: ClaimAssertionResult = { assertionId: 'assertion', status: 'supported', reasonCode: 'health_gate_failed', expected: null, observed: null };",
      "void artifactKind;",
