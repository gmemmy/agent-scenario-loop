...
function main(): void {
  ...
  try {
    ...
    const resolveSmokeScript = [
      ...
      "const os = require('os');",
      "const ciPackPath = path.join(os.tmpdir(), 'asl-ci-evidence-pack-package-smoke.json');",
      ...
      "  source: ciPackSource,",
      "  liveProofSet: { relativePath: 'live-proof-set.json', sha256: ciPackSha256, byteSize: 128, runId: 'run-1', status: 'passed' },",
      ...
      "};",
      "try {",
      "  const builtCiPack = asl.buildCiEvidencePack(ciPackInput);",
      ...
      "  assert.equal(readCiPack.twoPlatformClaim.status, 'passed');",
      "} finally {",
      "  fs.rmSync(ciPackPath, { force: true });",
      "}",
