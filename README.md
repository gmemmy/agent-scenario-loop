...
6. Run `asl-validate-project --root . --platform all` before live device proof. It checks the scaffolded helper, package-script snippets, app `package.json` script merge, scenarios, runners, local provider-command scripts, and planner compatibility, then reports warnings and structured next actions.
...
- add higher-fidelity provider recipes for specific external tools while keeping them optional
