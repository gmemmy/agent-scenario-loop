...
test('builds proof-set skipped interaction proof context from linked proofs', async (t: TestContext) => {
  ...
  androidProof.skippedInteractionProofs = [
    {
      ...
      profileGateDiagnostics: {
        ...
        providerEvidenceNextAction: {
          ...
          owner: 'provider_tooling',
          summary: 'Use preserved provider diagnostics for investigation only.',
        },
  ...
  assert.deepEqual(artifact.proofFailureProviderEvidenceNextActions, {
    failedProofCount: 2,
    nextActionCounts: [
      {
        code: 'use_partial_provider_evidence_for_diagnosis',
  ...
  assert.deepEqual(writtenJson.profileGateProviderEvidenceNextActions, artifact.profileGateProviderEvidenceNextActions);
  assert.deepEqual(writtenJson.proofFailureProviderEvidenceNextActions, artifact.proofFailureProviderEvidenceNextActions);
