...
test('builds proof-set skipped interaction proof context from linked proofs', async (t: TestContext) => {
  ...
  assert.match(markdown, /Diagnostics: captured=nativePerformance:diagnostic-only; blocking=accessibility:provider-blocked; nativePerformance\(claim=insufficient-for-claim, completeness=truncated, comparability=diagnostic-only, target=ambiguous, sources=xctrace:partial\)\./u);
