...
test('Argent capture executes scenario driver actions and writes artifacts', async (t: TestContext) => {
  ...
  assert.equal(fs.existsSync(path.join(tempDir, 'raw', 'argent-rotate-5.txt')), true);
...
test('Argent driver step expansion validates coordinate-backed metadata', () => {
  ...
  assert.deepEqual(resolveArgentDriverSteps(scenario), [
    ...
    },
    {
      ...
      endAngle: 90,
      radius: 0.25,
      rawFileName: 'argent-rotate-6.txt',
      required: true,
      startAngle: 0,
      stepId: 'rotate-map',
      waitMs: 0,
  ...
  assert.deepEqual(validateArgentDriverSteps([
    ...
    { driverAction: 'rotate', rawFileName: 'rotate.txt', required: true, stepId: 'rotate-missing', waitMs: 0 },
    ...
    'step `rotate-missing` uses driverAction `rotate` but is missing adapterOptions.argent.centerX/centerY/radius/startAngle/endAngle.',
