...
test('argent driver maps lifecycle, gestures, screenshots, and descriptions', async () => {
  ...
  const rotateGesture = await driver.rotateGesture({ centerX: 500, centerY: 800, durationMs: 500, endAngle: 90, radius: 0.25, startAngle: 0 });
  ...
  assert.equal(rotateGesture.action, 'rotateGesture');
