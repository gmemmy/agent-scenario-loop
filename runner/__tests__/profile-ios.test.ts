...
test('profile-ios downgrades capture-only native performance evidence to diagnostic-only', async (t: TestContext) => {
  ...
  await fsp.writeFile(
    ...
    [
      ...
      "  }],",
      "  diagnosticSources: [{ sourceId: 'xctrace', status: 'captured', dataClasses: ['frames', 'jank'], path: sourceEvidencePath }],",
      ...
      providerCommands: [
        {
          ...
          args: [
            ...
            `raw/providers/${providerId}/xctrace-summary.json`,
          ],
          outputs: [
            {
              channel: 'provider',
...
test('profile-ios classifies live-window native-performance evidence as comparison-ready from real runner command outputs', async (t: TestContext) => {
  ...
  await fsp.writeFile(
    ...
    [
      ...
      "const commandReference = ({ args, commandId, outputPath, outputSha256, phase, requestPath, requestSha256 }) => ({",
      '  sourceId: commandId,',
      '  commandId,',
      "  command: process.execPath,",
      '  args,',
