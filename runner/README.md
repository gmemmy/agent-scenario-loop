...
`examples/runners` includes adapter-target manifests for `agent-device` and axe-style accessibility evidence, plus `script-*` provider manifests for accessibility, profiler, memory, and network evidence. These fixtures let the planner prove capability matching and command-output inventory. For agent-device, the shipped driver adapter covers the portable interaction subset while broader tool surfaces still stay behind explicit future adapters or provider attachments.
