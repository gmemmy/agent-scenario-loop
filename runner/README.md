...
| --- | --- | --- |
| `android-adb-driver` | `tap`, `scroll`, `assertVisible`, `inspectTree`, `screenshot`, `record`, `readLogs` | Android-only adb driver actions plus explicit launch, deep-link, and log helpers |
...
Runner limits remain the same: the package does not boot or control simulators, provide generic consuming-app install or build orchestration, drive broad semantic UI workflows beyond supported driver actions, prove native preview surfaces without a runner/provider that declares `nativePreview`, or capture memory, network, or accessibility evidence from built-in drivers. Those capabilities belong in external adapters or provider commands that preserve the same ASL artifact contract.
