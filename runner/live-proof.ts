...
type LiveProofSetProofFailureReadinessRollup = LiveProofSetProfileGateReadinessRollup & {
  failedProofCount: number;
};
type LiveProofSetProfileGateReadinessNextActionsRollup = {
  nextActionCounts: LiveProofNextActionCount[];
  skippedInteractionProofCount: number;
};
type LiveProofSetProofFailureReadinessNextActionsRollup = (
  LiveProofSetProfileGateReadinessNextActionsRollup & {
    failedProofCount: number;
  }
);
...
type LiveProofSetArtifact = {
  ...
  profileGateReadinessNextActions?: LiveProofSetProfileGateReadinessNextActionsRollup;
  ...
  proofFailureReadinessNextActions?: LiveProofSetProofFailureReadinessNextActionsRollup;
...
function nativePerformanceTargetBindingDetailKey(
...
function nativePerformanceClaimSufficiencyDetailKey(
...
function incrementNativePerformanceClaimSufficiencyDetailCount(
  counts: Map<string, LiveProofNativePerformanceClaimSufficiencyDetailCount>,
  detail: LiveProofNativePerformanceClaimSufficiencyDetail,
  count: number,
): void {
  const key = nativePerformanceClaimSufficiencyDetailKey(detail);
...
function incrementNativePerformanceTargetBindingDetailCount(
  counts: Map<string, LiveProofNativePerformanceTargetBindingDetailCount>,
  detail: LiveProofNativePerformanceTargetBindingDetail,
  count: number,
): void {
  const key = nativePerformanceTargetBindingDetailKey(detail);
  const existing = counts.get(key);
  if (existing) {
    existing.count += count;
    return;
...
function addNativePerformanceClaimSufficiencyDetailCounts(
  counts: Map<string, LiveProofNativePerformanceClaimSufficiencyDetailCount>,
  entries: LiveProofNativePerformanceClaimSufficiencyDetailCount[] | undefined,
): void {
  for (const entry of entries ?? []) {
    incrementNativePerformanceClaimSufficiencyDetailCount(counts, entry, entry.count);
  }
}

function addNativePerformanceClaimSufficiencyDetails(
  counts: Map<string, LiveProofNativePerformanceClaimSufficiencyDetailCount>,
  entries: LiveProofNativePerformanceClaimSufficiencyDetail[] | undefined,
): void {
  for (const entry of entries ?? []) {
    incrementNativePerformanceClaimSufficiencyDetailCount(counts, entry, 1);
  }
}

/**
 * Adds native-performance target-binding detail counts into a proof-set accumulator.
 *
 * @param {Map<string, LiveProofNativePerformanceTargetBindingDetailCount>} counts
 * @param {LiveProofNativePerformanceTargetBindingDetailCount[] | undefined} entries
 * @returns {void}
 */
function addNativePerformanceTargetBindingDetailCounts(
  counts: Map<string, LiveProofNativePerformanceTargetBindingDetailCount>,
  entries: LiveProofNativePerformanceTargetBindingDetailCount[] | undefined,
): void {
  for (const entry of entries ?? []) {
    incrementNativePerformanceTargetBindingDetailCount(counts, entry, entry.count);
  }
}

/**
 * Adds one-count native-performance target-binding details into a proof-set accumulator.
 *
 * @param {Map<string, LiveProofNativePerformanceTargetBindingDetailCount>} counts
 * @param {LiveProofNativePerformanceTargetBindingDetail[] | undefined} entries
 * @returns {void}
 */
function addNativePerformanceTargetBindingDetails(
  counts: Map<string, LiveProofNativePerformanceTargetBindingDetailCount>,
  entries: LiveProofNativePerformanceTargetBindingDetail[] | undefined,
): void {
  for (const entry of entries ?? []) {
    incrementNativePerformanceTargetBindingDetailCount(counts, entry, 1);
  }
}
...
/**
 * Converts accumulated native-performance target-binding detail counts into stable artifact order.
 *
 * @param {Map<string, LiveProofNativePerformanceTargetBindingDetailCount>} counts
...
function buildLiveProofSetNativePerformanceRollup(
  ...
  for (const proof of proofs) {
    ...
    addNativePerformanceClaimSufficiencyDetailCounts(claimSufficiencyDetailCounts, rollup.claimSufficiencyDetailCounts);
    addNativePerformanceStatusCounts(completenessStatusCounts, rollup.completenessStatusCounts);
    ...
    addNativePerformanceTargetBindingDetailCounts(targetBindingDetailCounts, rollup.targetBindingDetailCounts);
  }
  ...
  const claimDetailCounts = formatNativePerformanceClaimSufficiencyDetailCounts(claimSufficiencyDetailCounts);
  const completenessCounts = formatNativePerformanceStatusCounts(completenessStatusCounts);
  ...
  const targetDetailCounts = formatNativePerformanceTargetBindingDetailCounts(targetBindingDetailCounts);
  return {
...
function buildLiveProofSetProfileGateNativePerformanceRollup(
  ...
  for (const skippedProof of skippedProofs) {
    ...
    addNativePerformanceClaimSufficiencyDetails(
      claimSufficiencyDetailCounts,
      nativePerformance.claimSufficiencyDetails,
    );
    if (nativePerformance.completenessStatus) {
      completenessStatusCounts.set(
        nativePerformance.completenessStatus,
        (completenessStatusCounts.get(nativePerformance.completenessStatus) ?? 0) + 1,
      );
    }
    ...
    addNativePerformanceTargetBindingDetails(targetBindingDetailCounts, nativePerformance.targetBindingDetails);
  }
  ...
  if (
    ...
    claimDetailCounts.length === 0 &&
    completenessCounts.length === 0 &&
    comparableCounts.length === 0 &&
    sourceCounts.length === 0 &&
    targetCounts.length === 0 &&
    targetDetailCounts.length === 0
  ) {
    return null;
  }
...
function hasNativePerformanceProfileGateContext(
  ...
  return Boolean(
    nativePerformance.claimSufficiency ||
    (nativePerformance.claimSufficiencyDetails?.length ?? 0) > 0 ||
    nativePerformance.completenessStatus ||
    nativePerformance.comparability ||
    nativePerformance.targetBinding ||
    (nativePerformance.targetBindingDetails?.length ?? 0) > 0 ||
...
/**
 * Builds profile-session readiness counts across failed linked proof artifacts.
 *
 * @param {{proofs: LiveProofArtifact[], skippedProofs: LiveProofSetSkippedInteractionProofPointer[]}} options
 * @returns {LiveProofSetProofFailureReadinessRollup | null}
 */
function buildLiveProofSetProofFailureReadinessRollup({
  proofs,
  skippedProofs,
}: {
  proofs: LiveProofArtifact[];
  skippedProofs: LiveProofSetSkippedInteractionProofPointer[];
}): LiveProofSetProofFailureReadinessRollup | null {
  const failedProofRunIds = new Set(
    proofs
      .filter((proof) => proof.status === 'failed')
      .map((proof) => proof.runId),
  ...
  const failedProofsWithReadiness = new Set<string>();
  const readinessSkippedProofs = skippedProofs.filter((skippedProof) => {
    if (!failedProofRunIds.has(skippedProof.proofRunId) || !skippedProof.profileGateReadiness) {
      return false;
    }
    failedProofsWithReadiness.add(skippedProof.proofRunId);
    return true;
  });
  const rollup = buildLiveProofSetProfileGateReadinessRollup(readinessSkippedProofs);
  if (!rollup) {
    return null;
  }

  return {
    failedProofCount: failedProofsWithReadiness.size,
    ...rollup,
  };
}

/**
 * Builds profile-session readiness next-action counts across skipped proof gates.
 *
 * @param {LiveProofSetSkippedInteractionProofPointer[]} skippedProofs
 * @returns {LiveProofSetProfileGateReadinessNextActionsRollup | null}
 */
function buildLiveProofSetProfileGateReadinessNextActionsRollup(
  skippedProofs: LiveProofSetSkippedInteractionProofPointer[],
): LiveProofSetProfileGateReadinessNextActionsRollup | null {
...
/**
 * Builds profile-session readiness next-action counts across failed linked proofs.
 *
 * @param {{proofs: LiveProofArtifact[], skippedProofs: LiveProofSetSkippedInteractionProofPointer[]}} options
 * @returns {LiveProofSetProofFailureReadinessNextActionsRollup | null}
 */
function buildLiveProofSetProofFailureReadinessNextActionsRollup({
  proofs,
  skippedProofs,
}: {
  proofs: LiveProofArtifact[];
  skippedProofs: LiveProofSetSkippedInteractionProofPointer[];
}): LiveProofSetProofFailureReadinessNextActionsRollup | null {
  const failedProofRunIds = new Set(
    proofs
      .filter((proof) => proof.status === 'failed')
      .map((proof) => proof.runId),
  ...
  const failedProofsWithReadinessNextAction = new Set<string>();
  const readinessSkippedProofs = skippedProofs.filter((skippedProof) => {
    if (
      !failedProofRunIds.has(skippedProof.proofRunId) ||
      !skippedProof.profileGateReadiness?.readinessNextAction
    ) {
      return false;
    }
    failedProofsWithReadinessNextAction.add(skippedProof.proofRunId);
    return true;
  });
  const rollup = buildLiveProofSetProfileGateReadinessNextActionsRollup(readinessSkippedProofs);
  if (!rollup) {
    return null;
  }

  return {
    failedProofCount: failedProofsWithReadinessNextAction.size,
    ...rollup,
  };
}
...
function buildLiveProofSetArtifact({
  ...
  const proofFailureReadiness = buildLiveProofSetProofFailureReadinessRollup({
    proofs,
    skippedProofs: skippedInteractionProofs,
  });
  const profileGateReadinessNextActions = buildLiveProofSetProfileGateReadinessNextActionsRollup(
    skippedInteractionProofs,
  );
  const proofFailureReadinessNextActions = buildLiveProofSetProofFailureReadinessNextActionsRollup({
    proofs,
    skippedProofs: skippedInteractionProofs,
  });
  ...
  return {
    ...
    ...(proofFailureReadinessNextActions ? { proofFailureReadinessNextActions } : {}),
...
function formatLiveProofSetProfileGateNativePerformanceRollup(
  ...
  if (rollup.completenessStatusCounts?.length) {
    details.push(`completeness=${formatLiveProofSetNativePerformanceStatusCounts(rollup.completenessStatusCounts)}`);
  }
  ...
  if (rollup.claimSufficiencyDetailCounts?.length) {
    details.push(`claimDetails=${formatLiveProofSetNativePerformanceClaimSufficiencyDetailCounts(rollup.claimSufficiencyDetailCounts)}`);
  }
  ...
  if (rollup.targetBindingDetailCounts?.length) {
    details.push(`targetDetails=${formatLiveProofSetNativePerformanceTargetBindingDetailCounts(rollup.targetBindingDetailCounts)}`);
  }
...
/**
 * Formats aggregate failed-proof readiness counts for proof-set markdown.
 *
 * @param {LiveProofSetProofFailureReadinessRollup | undefined} rollup
 * @returns {string[]}
 */
function formatLiveProofSetProofFailureReadinessRollup(
  rollup: LiveProofSetProofFailureReadinessRollup | undefined,
): string[] {
  if (!rollup) {
    return [];
  }
  ...
  return [
    `Proof failure readiness: ${details.join('; ')}`,
  ];
}
...
/**
 * Formats profile-session readiness next-action counts across failed proofs.
 *
 * @param {LiveProofSetProofFailureReadinessNextActionsRollup | undefined} rollup
 * @returns {string[]}
 */
function formatLiveProofSetProofFailureReadinessNextActionsRollup(
  rollup: LiveProofSetProofFailureReadinessNextActionsRollup | undefined,
): string[] {
  if (!rollup) {
    return [];
  }
  ...
  return [
    `Proof failure readiness next actions: failedProofs=${rollup.failedProofCount}; skippedInteractionProofs=${rollup.skippedInteractionProofCount}; actions=${formatLiveProofSetNextActionCountsForMarkdown(rollup.nextActionCounts)}`,
  ];
}

/**
 * Formats profile-session readiness next-action counts across skipped proof gates.
 *
 * @param {LiveProofSetProfileGateReadinessNextActionsRollup | undefined} rollup
 * @returns {string[]}
 */
function formatLiveProofSetProfileGateReadinessNextActionsRollup(
  rollup: LiveProofSetProfileGateReadinessNextActionsRollup | undefined,
): string[] {
  if (!rollup) {
    return [];
  }
  ...
  return [
    `Profile gate readiness next actions: skippedInteractionProofs=${rollup.skippedInteractionProofCount}; actions=${formatLiveProofSetNextActionCountsForMarkdown(rollup.nextActionCounts)}`,
  ];
}
...
function formatLiveProofSetNativePerformanceClaimSufficiencyDetails(
  ...
  if (nativePerformance) {
    const nativeParts = [
      ...
      nativePerformance.claimSufficiencyDetails?.length
        ? `claimDetails=${formatLiveProofSetNativePerformanceClaimSufficiencyDetails(nativePerformance.claimSufficiencyDetails)}`
        : '',
      ...
      nativePerformance.targetBindingDetails?.length
        ? `targetDetails=${formatLiveProofSetNativePerformanceTargetBindingDetails(nativePerformance.targetBindingDetails)}`
        : '',
...
function formatLiveProofSetProfileGateReadiness(
  ...
  if (readiness.readinessNextAction) {
    parts.push(`readinessNextAction=${readiness.readinessNextAction.owner ?? 'unknown'}/${readiness.readinessNextAction.code}`);
  }
...
function formatLiveProofSetNativePerformanceTargetBindingDetails(
...
/**
 * Formats native-performance target-binding detail counts for proof-set markdown.
 *
 * @param {LiveProofNativePerformanceTargetBindingDetailCount[]} counts
 * @returns {string}
 */
function formatLiveProofSetNativePerformanceTargetBindingDetailCounts(
...
function formatLiveProofSetNativePerformanceClaimSufficiencyDetailCounts(
...
function formatLiveProofSetArtifactMarkdown(artifact: LiveProofSetArtifact): string {
  return [
    ...
    ...formatLiveProofSetProofFailureReadinessNextActionsRollup(artifact.proofFailureReadinessNextActions),
    ...
    ...formatLiveProofSetProfileGateReadinessNextActionsRollup(artifact.profileGateReadinessNextActions),
