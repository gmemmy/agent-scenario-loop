...
type ArgentDriverStep = {
  ...
  centerX?: number;
  centerY?: number;
  driverAction: 'assertVisible' | 'drag' | 'inspectTree' | 'launch' | 'longPress' | 'pinch' | 'rotateGesture' | 'screenshot' | 'scroll' | 'swipe' | 'tap';
  durationMs?: number;
  endAngle?: number;
  endDistance?: number;
  endX?: number;
...
/**
 * Returns true when an Argent driver step has explicit start and end coordinates.
 *
 * @param {ArgentDriverStep} step
 * @returns {boolean}
 */
function hasArgentStartEndCoordinates(step: ArgentDriverStep): boolean {
  return (
...
/**
 * Returns true when an Argent rotate gesture step has the tool-native metadata.
 *
 * @param {ArgentDriverStep} step
 * @returns {boolean}
 */
function hasArgentRotateGestureMetadata(step: ArgentDriverStep): boolean {
  return (
    typeof step.centerX === 'number' &&
    typeof step.centerY === 'number' &&
 ...
 * @returns {boolean}
 */
function isArgentStartEndAction(driverAction: ArgentDriverStep['driverAction']): boolean {
...
function resolveArgentDriverSteps(
  ...
  return executionPlan.steps
    .filter((step: ScenarioExecutionStep) =>
      step.kind === 'launch' ||
      ['assertVisible', 'drag', 'inspectTree', 'longPress', 'pinch', 'rotateGesture', 'screenshot', 'scroll', 'swipe', 'tap'].includes(String(step.driverAction)),
    )
...
function validateArgentDriverSteps(driverSteps: ArgentDriverStep[], options: {app?: string | null} = {}): string[] {
  ...
  for (const step of driverSteps) {
    ...
    if (step.driverAction === 'rotateGesture' && !hasArgentRotateGestureMetadata(step)) {
      errors.push(`${stepLabel} uses driverAction \`rotateGesture\` but is missing adapterOptions.argent.centerX/centerY/radius/startAngle/endAngle.`);
    }
...
async function runArgentDriverStep({
  ...
  if (driverStep.driverAction === 'drag') {
    return driver.drag({
      ...(typeof driverStep.durationMs === 'number' ? { durationMs: driverStep.durationMs } : {}),
      endX: driverStep.endX as number,
      endY: driverStep.endY as number,
  ...
  if (driverStep.driverAction === 'rotateGesture') {
    return driver.rotateGesture({
      centerX: driverStep.centerX as number,
      centerY: driverStep.centerY as number,
      ...(typeof driverStep.durationMs === 'number' ? { durationMs: driverStep.durationMs } : {}),
