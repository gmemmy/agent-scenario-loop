...
type ArgentDriver = {
  ...
  rotateGesture: (options: ArgentRotateOptions) => Promise<ArgentCommandResult>;
...
function createArgentDriver(options: ArgentDriverOptions): ArgentDriver {
  ...
  return {
    ...
    async rotateGesture({
      ...
      endAngle,
      radius,
      rawFileName = 'argent-rotate-gesture.txt',
      screenSize,
      startAngle,
      ...
      return run('rotateGesture', 'gesture-rotate', rawFileName, [
        '--centerX',
        String(center.x),
        '--centerY',
        String(center.y),
