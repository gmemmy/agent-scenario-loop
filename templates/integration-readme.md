...
Run `pnpm asl:host:doctor` before host/device live proof when adb, CoreSimulator, agent-device, Argent, Metro, a local debug host, or artifact storage capacity is uncertain. Set `ASL_HOST_DOCTOR_REQUIRE=android,ios,agent-device,argent` in `.asl.local.env` or the shell when sidecar proof availability is part of the gate. Set `ASL_HOST_DOCTOR_TCP_PORTS=localhost:8081` or pass `--tcp-port localhost:8081` when a live proof depends on a warm Metro/dev-server port. Set `ASL_HOST_DOCTOR_MIN_FREE_DISK=artifacts/asl:1024` or pass `--min-free-disk artifacts/asl:1024` before trace-heavy, video, or native-diagnostics proof.
