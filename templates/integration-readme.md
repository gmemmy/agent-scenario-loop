...
- `src/devtools/profile-session-storage.ts`: profile-session storage helper
- `src/devtools/profile-session-command-ordering.ts`: profile-session command ordering helper
- `src/devtools/profile-session-dependency-controller.ts`: profile-session dependency controller
- `src/devtools/profile-session-authoritative-storage.ts`: profile-session authoritative storage helper
- `src/devtools/profile-session-helper.json`: profile-session helper identity JSON
...
the missing files. Reconcile or replace the complete helper source set
...
1. Install the peer app dependencies used by the generated `src/devtools/` helper source set if your app does not already have them:
...
Set `ASL_ADB_PATH` when `adb` is not on `PATH`, set `ASL_XCRUN_PATH` when `xcrun` is not on `PATH`, and use `ASL_NATIVE_PERFORMANCE_COMMAND_TIMEOUT_MS` only when the Android default 10-second per-command bound is unsuitable. iOS capture accepts `ASL_NATIVE_PERFORMANCE_IOS_CAPTURE_DURATION_SECONDS`, `ASL_NATIVE_PERFORMANCE_IOS_XCTRACE_TEMPLATE` (default `Time Profiler`), and `ASL_NATIVE_PERFORMANCE_IOS_COMMAND_TIMEOUT_MS`; its command-timeout default is the requested capture duration plus 30 seconds so xctrace can finalize and save the bundle. On live adb/simctl runs, ASL now also stages `raw/native-performance-request.json` before `startWindow`; the starter native-performance manifest passes that path as `{nativePerformanceRequestPath}` and its staged hash as `{nativePerformanceRequestSha256}` so a project-local provider can validate the requested app/target identity and the runner-owned `activeLoop` window policy before it starts capture, then fail closed if the staged request bytes drift later in the lifecycle. Provider commands use optional `platforms` to keep Android and iOS capture commands out of each other's plans. On Android, `startWindow` resets the `gfxinfo` window, `stopWindow` writes direct target/gfxinfo/framestats/meminfo outputs, and `afterCapture` normalizes those preserved bytes without recollection; only completed zero-exit/no-timeout/no-signal runner records with exact argv and matching fresh-output hashes can establish capture provenance. The provider preserves stdout, stderr, argv, exit status, timeouts, source status, and target-verification evidence under its run folder, including run-relative TOC, target-binding, and trace-bundle inventory files for `.trace` directory captures. The starter lanes can be complete enough for diagnosis when target/window checks succeed, but they are not sufficient for comparison claims without structured native performance samples, explicit policy, and trusted compatible baselines.
