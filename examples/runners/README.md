...
- Rich scenario actions such as `longPress`, `drag`, `pinch`, `rotate`, `typeText`, `pressKey`, `pressButton`, `customGesture`, and `runSequence` are valid vocabulary but not automatically supported by these fixtures. `swipe` is declared only by adapter fixtures that map it to bounded command transcripts.
