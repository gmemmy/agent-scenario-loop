...
- Rich scenario actions such as `drag`, `pinch`, `rotate`, `pressKey`, `pressButton`, `customGesture`, and `runSequence` are valid vocabulary but not automatically supported by these fixtures. `longPress`, `typeText`, and `swipe` are declared only by adapter fixtures that map them to bounded command transcripts.
