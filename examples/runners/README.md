...
Native performance providers should translate platform-native evidence into a product-neutral `nativePerformance` attachment. On Android, that can include Perfetto traces, trace-processor summaries, `gfxinfo`/framestats, `meminfo`, and logcat-derived render signals. On iOS, it can include Instruments, MetricKit, or simctl-derived native performance summaries. Keep raw traces as attachments and put claim-ready facts in a structured provider summary; do not collapse native frame or memory evidence into generic profiler output unless the provider also records the native tool, clock window, device/app binding, and comparability policy.
