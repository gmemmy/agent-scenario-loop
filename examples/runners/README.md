...
- Rich scenario actions such as `drag`, `pinch`, `rotate`, `fill`, `focus`, `pressKey`, `pressButton`, `customGesture`, and `runSequence` are valid vocabulary but not automatically supported by every fixture. `longPress`, `drag`, `typeText`, `fill`, `focus`, `swipe`, `rotate`, `pressKey`, `pressButton`, and `pinch` are declared only by adapter fixtures that map them to bounded command transcripts.
